...
function setAutoScroll(v){
  ...
  if(turningOn && playback && playback.isPlaying){
    const t = (playerEl && typeof playerEl.currentTime === "number") ? playerEl.currentTime : 0;
    const bpmNow = +(((getActiveProject && getActiveProject())?.bpm) || (els.bpm && els.bpm.value) || 120);
    playback.beatOffsetBeats = (t * bpmNow) / 60;
    // Anchor to the currently active page so we start on the first card on-screen
    playback.anchorPageKey = (getActiveProject && getActiveProject())?.activeSection || null;
  }
  if(!autoScrollOn && playback){
...
function computeSeqStartOffsetFromViewport(p){
  try{
    const seq = getPlaySequence(p);
    if (!seq || !seq.length) return 0;
    const pageKey = p.activeSection || (seq[0] && seq[0].sectionKey) || "1";
    const barsScroller = document.getElementById("bars");
    const pageEl = document.querySelector(`.page[data-page-key="${pageKey}"]`);
    if (!barsScroller || !pageEl) {
      const idx = seq.findIndex(x => x.sectionKey === pageKey);
      return idx >= 0 ? idx : 0;
    }
    const cRect = barsScroller.getBoundingClientRect();
    const bars = Array.from(pageEl.querySelectorAll(".bar"));
    let best = null;
    for (const el of bars){
      const r = el.getBoundingClientRect();
      const vis = (r.bottom > cRect.top + 8) && (r.top < cRect.bottom - 8);
      if (!vis) continue;
      if (!best || r.top < best.r.top) best = {el, r};
    }
    const barIdx = best ? Number(best.el.dataset.barIdx||0) : 0;
    let seqIdx = seq.findIndex(x => x.sectionKey === pageKey && Number(x.barIdx) === barIdx);
    if (seqIdx < 0) seqIdx = seq.findIndex(x => x.sectionKey === pageKey);
    return seqIdx >= 0 ? seqIdx : 0;
  }catch(e){
    return 0;
  }
}
