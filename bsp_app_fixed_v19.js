...
function setAutoScroll(v){
  ...
  if(!autoScrollOn){
    ...
    if(p && typeof saveProject === "function"){
      try{ saveProject(p); }catch(e){}
    }
    return;
  }

  if(turningOn){
    // Anchor to the currently active page and the first visible bar on-screen.
    if(p){
      p.playback.anchorPageKey = p.activeSection || p.playback.anchorPageKey || null;
      p.playback.seqStartOffset = computeSeqStartOffsetFromViewport(p);
    }
...
function getFirstVisibleBarIdxInActivePage(){
  try{
    const pageEl = (typeof getActiveRealPageEl === "function") ? getActiveRealPageEl() : null;
    if(!pageEl) return 0;

    const scroller = (typeof findVerticalScroller === "function") ? (findVerticalScroller(pageEl) || document.scrollingElement) : document.scrollingElement;
    const bars = Array.from(pageEl.querySelectorAll('.bar[data-bar-idx], .barCard[data-bar-idx]'));
    if(!bars.length) return 0;

    const viewTop = (scroller && scroller !== document.body && scroller !== document.documentElement && scroller !== document.scrollingElement)
      ? scroller.getBoundingClientRect().top
      : 0;
    const viewBottom = (scroller && scroller !== document.body && scroller !== document.documentElement && scroller !== document.scrollingElement)
      ? scroller.getBoundingClientRect().bottom
      : window.innerHeight;

    for(const el of bars){
      const r = el.getBoundingClientRect();
      if(r.bottom > viewTop + 10 && r.top < viewBottom - 10){
...
function startPracticeScroll(p){
  ...
  practiceBeat16 = 0;
  practiceStartBarIdx = getFirstVisibleBarIdxInActivePage();

  const bpmRaw = (p && typeof p.bpm !== "undefined") ? p.bpm : (els?.bpmInput ? parseFloat(els.bpmInput.value) : 90);
  ...
  practiceScrollTimer = setInterval(() => {
    ...
    const pageKey = activeProject.activeSection || activeProject.activePageKey || (activeProject.pageOrder ? activeProject.pageOrder[0] : null) || null;

    const step16 = practiceBeat16 % 16;
    const beatInBar = Math.floor(step16 / 4) + 1;
    const barIdx = practiceStartBarIdx + Math.floor(practiceBeat16 / 16);

    // keep in sync with existing autoscroll engine
    if(typeof syncHighlightAndScroll === "function"){
      syncHighlightAndScroll(activeProject, pageKey, barIdx, beatInBar);
    }
