...
function setAutoScroll(v){
  ...
  if(p){
    const visibleKeyNow = getVisibleRealPageKeyFromPager(p) || p.activeSection || "full";
...
function ensureFullViewMode(p, wantPlay){
  try{
    const fullPage = getPageElForKey(p, "full");
    if(!fullPage) return;
    const mount = fullPage.querySelector(".fullSongMount");
    if(!mount) return;
    ...
    const scroller = findVerticalScroller(mount) || document.scrollingElement;
