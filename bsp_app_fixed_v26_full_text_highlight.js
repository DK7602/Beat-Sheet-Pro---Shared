...
function setAutoScroll(v){
  ...
  if(turningOn){
    ...
    if(p){
      ...
      p.playback.seqStartOffset = (visibleKey === "full")
        ? computeSeqStartOffsetFromViewport(p)
        : computeSeqStartOffsetFromAnchor(p, visibleKey, firstVisibleBarIdx);
    }
  ...
  // If the user is viewing FULL, switch FULL between edit/play modes so highlighting works.
  if(p){
    let visibleKeyNow = null;
    try{
      const pagerEl = document.getElementById("pagesPager") || (els?.bars?.querySelector?.(".pager"));
      const vis = pagerEl ? (pagerEl.children && pagerEl.children[getCurrentIdx(pagerEl)]) : null;
      visibleKeyNow = (vis && vis.dataset) ? vis.dataset.pageKey : null;
    }catch(e){}
    visibleKeyNow = visibleKeyNow || getVisibleRealPageKeyFromPager(p) || p.activeSection || "full";

    if(visibleKeyNow === "full"){
      ensureFullViewMode(p, !!autoScrollOn);
      // also keep activeSection in sync so other render passes keep FULL in play mode
      if(p.activeSection !== "full") p.activeSection = "full";
    }else{
      // keep FULL in edit mode when user isn't on it
      ensureFullViewMode(p, false);
    }
  }
...
/***********************
✅ FULL VIEW MODE (edit + highlight overlay)
- When AutoScroll is ON and user is on FULL, we keep the normal Full text view
  but render a transparent highlight overlay that flashes the beat-matched text segments.
***********************/
function ensureFullViewMode(p, wantPlay){
  try{
    const pagerEl = document.getElementById("pagesPager") || (els?.bars?.querySelector?.(".pager"));
    let fullPage = null;

    // Prefer the currently visible pager page if it's FULL
    const vis = pagerEl ? (pagerEl.children && pagerEl.children[getCurrentIdx(pagerEl)]) : null;
    if(vis && vis.dataset && vis.dataset.pageKey === "full"){
      fullPage = vis;
    }

    // Fallback: any FULL page (real or clone)
    if(!fullPage){
      fullPage = document.querySelector('.page[data-page-key="full"]');
    }
    if(!fullPage) return;

    // Ensure a mount exists
    let mount = fullPage.querySelector(".fullSongMount");
    if(!mount){
      mount = document.createElement("div");
      mount.className = "fullSongMount fullSongFlow";
      fullPage.appendChild(mount);
    }

    const curPlay = (mount.dataset && mount.dataset.fullMode === "highlight");
    const nextPlay = !!wantPlay;
    if(curPlay === nextPlay) return;

    // preserve vertical scroll position inside FULL page
    const scroller = findVerticalScroller(mount) || fullPage || document.scrollingElement;
    const prevTop = scroller ? (scroller.scrollTop || 0) : 0;

    renderFullSongFlow(p, mount, { highlightMode: nextPlay });

    requestAnimationFrame(()=>{
      try{ if(scroller) scroller.scrollTop = prevTop; }catch(e){}
    });
  }catch(e){}
}
...
function getFirstVisibleBarIdxInActivePage(){
  ...
  try{
    ...
    const pageEl = (typeof getPageElForKey === "function") ? getPageElForKey(p, key) : ((typeof getActiveRealPageEl === "function") ? getActiveRealPageEl(key) : null);
    if(!pageEl) return 0;
...
function startPracticeScroll(p){
  ...
  if(activeProject){
    ...
    const firstVisibleBarIdx = getFirstVisibleBarIdxInActivePage();
    activeProject.playback.seqStartOffset = (activeProject.playback.anchorPageKey === "full")
      ? computeSeqStartOffsetFromViewport(activeProject)
      : computeSeqStartOffsetFromAnchor(activeProject, activeProject.playback.anchorPageKey, firstVisibleBarIdx);
  }
...
function escSel(s){
  // safe selector escape (CSS.escape may not exist on some builds)
  try{
    if(window.CSS && typeof CSS.escape === "function") return escSel(String(s));
  }catch(e){}
  return String(s).replace(/[^a-zA-Z0-9_\-]/g, "\\$&");
}

// Prefer the currently visible page (even if it's a carousel clone).
function getVisiblePageElFromPager(p){
  try{
    const pagerEl = document.getElementById("pagesPager") || (els?.bars?.querySelector?.(".pager"));
    if(!pagerEl) return null;
    const idx = getCurrentIdx(pagerEl);
    return pagerEl.children && pagerEl.children[idx] ? pagerEl.children[idx] : null;
  }catch(e){
    return null;
  }
}
...
function getActiveRealPageEl(pageKey){
  return document.querySelector(`.page[data-page-key="${escSel(pageKey)}"]:not([data-clone="1"])`);
}

function getPageElForKey(p, pageKey){
  const vis = getVisiblePageElFromPager(p);
  if(vis && vis.dataset && vis.dataset.pageKey === pageKey) return vis;
  return getActiveRealPageEl(pageKey);
}

function clearOldActiveBar(){
  if(lastActiveBarKey == null) return;
  const baseKey = String(lastActiveBarKey).split(":")[0];
  const oldPage = getActiveRealPageEl(baseKey);
  if(oldPage){
...
function setActiveBarDOM(pageKey, barIdx, subSecKey=null){
  clearOldActiveBar();

  // base pageKey (FULL may include a sub-section)
  const baseKey = String(pageKey || "").split(":")[0] || "full";

  // Prefer visible pager page (handles carousel clones)
  let page = null;
  try{
    const pagerEl = document.getElementById("pagesPager") || (els?.bars?.querySelector?.(".pager"));
    const vis = pagerEl ? (pagerEl.children && pagerEl.children[getCurrentIdx(pagerEl)]) : null;
    if(vis && vis.dataset && vis.dataset.pageKey === baseKey) page = vis;
  }catch(e){}

  page = page || getPageElForKey(getActiveProject(), baseKey) || document.querySelector(`.page[data-page-key="${escSel(baseKey)}"]`);
  if(!page) return null;

  let bar = null;

  if(baseKey === "full"){
    // Full-view highlight overlay: bars are .fullOverlayLine inside each .fullSection
    const sk = String(subSecKey || "").trim();
    if(!sk) return null;

    const secEl = page.querySelector(`.fullSection[data-sec-key="${escSel(sk)}"]`);
    if(!secEl) return null;

    bar = secEl.querySelector(`.fullOverlayLine[data-bar-idx="${barIdx}"]`);
  }else{
    bar = page.querySelector(`.bar[data-bar-idx="${barIdx}"]`);
  }

  if(!bar) return null;

  bar.classList.add("barActive");
  lastActiveBarKey = (baseKey === "full" && subSecKey) ? `full:${subSecKey}` : baseKey;
...
function flashBeatOnBar(barEl, beatInBar){
  ...
  // Card/beat-box bars
  const boxBeats = barEl.querySelectorAll(".beat");
  if(boxBeats && boxBeats.length >= 4){
    boxBeats.forEach(b=>b.classList.remove("flash"));
    const t = boxBeats[beatInBar];
    if(t) t.classList.add("flash");
    setTimeout(()=>boxBeats.forEach(b=>b.classList.remove("flash")), 90);
    return;
  }

  // FULL highlight overlay bars
  const spans = barEl.querySelectorAll(".fullBeatSpan");
  if(!spans || !spans.length) return;
  spans.forEach(s=>s.classList.remove("flash"));
  const t = barEl.querySelector(`.fullBeatSpan[data-beat-idx="${beatInBar}"]`);
  if(t) t.classList.add("flash");
  setTimeout(()=>{
    try{ barEl.querySelectorAll(".fullBeatSpan.flash").forEach(s=>s.classList.remove("flash")); }catch(e){}
  }, 90);
}

function flashBeatOnAllBars(pageKey, beatInBar){
  const page = getPageElForKey(getActiveProject(), pageKey);
  if(!page) return;
  ...
  setTimeout(()=>{
    const page2 = getPageElForKey(getActiveProject(), pageKey);
    if(!page2) return;
...
function clearAllPracticeAndActive(){
  ...
  document.querySelectorAll(".fullBeatSpan.flash").forEach(b=>b.classList.remove("flash"));
...
function computeSeqStartOffsetFromViewport(p){
  try{
    ...
    // Scroller that contains the cards / full flow
    const scroller =
    ...
    const barIdx = parseInt(first.getAttribute('data-bar-idx') || (first.dataset ? first.dataset.barIdx : '0') || '0', 10) || 0;

    // ✅ FULL view: bars live inside sections; map to (secKey, barIdx)
    if(pageKey === "full"){
      const secEl = first.closest(".fullSection");
      const secKey = secEl ? (secEl.dataset.secKey || secEl.getAttribute("data-sec-key") || "") : "";
      if(secKey){
        const offFull = seq.findIndex(x => (x.secKey === secKey || x.sectionKey === secKey) && Number(x.barIdx) === Number(barIdx));
        return offFull >= 0 ? offFull : 0;
      }
      return 0;
    }
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  // If user is on FULL and AutoScroll is ON, render FULL in play mode so we can highlight.
  const userOnFull = (p.activeSection === "full");
  if(autoScrollOn && userOnFull){
    ensureFullViewMode(p, true);
  }

  // ✅ AutoScroll OFF:
  // - card pages: tick-highlight ALL bars on active page
  // - FULL: no beat boxes in edit mode, so do nothing
  if(!autoScrollOn){
    if(pageKey === "full" || userOnFull) return;
  ...
  if(!seq.length){
    // FULL can't do per-section loop; nothing to highlight
    if(pageKey === "full" || userOnFull) return;
  ...
  // If user is NOT on FULL, ensure we are on the right page (auto-advance pages that have text)
  if(!userOnFull && p.activeSection !== target.secKey){
    gotoSectionKey(p, target.secKey, "smooth");
    clearAllPracticeAndActive();
  }

  // Highlight the correct bar:
  // - If user is on FULL: highlight inside FULL under the matching section
  // - Else: highlight on the target section page
  const highlightOnFull = userOnFull || pageKey === "full";
  const barEl = highlightOnFull
    ? setActiveBarDOM("full", target.barIdx, target.secKey)
    : setActiveBarDOM(target.secKey, target.barIdx);

  if(barEl) flashBeatOnBar(barEl, safeBeat);

  // scroll once per (section,bar) on beat 1
  if(barEl && safeBeat === 0){
    const token = highlightOnFull ? `full:${target.secKey}:${target.barIdx}` : `${target.secKey}:${target.barIdx}`;
    if(token !== lastAutoScrollToken){
      lastAutoScrollToken = token;
      requestAnimationFrame(()=>scrollBarIntoView(barEl));
    }
...
/***********************
✅ FULL highlight overlay helpers (BSP)
- Keeps the normal Full song text layout, but allows per-beat flashing by rendering a transparent overlay
  with spans aligned to the same text using the SAME computeBeats() split logic used on cards.
***********************/
let __fullOverlayStylesAdded = false;
function ensureFullOverlayStyles(){
  if(__fullOverlayStylesAdded) return;
  __fullOverlayStylesAdded = true;
  const css = `
    .fullEditorWrap{ position:relative; }
    .fullHighlightOverlay{
    ...
    .fullOverlayLine{ white-space:pre-wrap; }
    .fullBeatSpan{ display:inline; border-radius:6px; }
    .fullBeatSpan.flash{
  ...
  style.id = "bspFullOverlayStyles";
  style.textContent = css;
  document.head.appendChild(style);
}

function buildFullHighlightHTML(p, secKey){
  const key = String(secKey || "");
  const sec = p?.sections?.[key];
  const bars = Array.isArray(sec?.bars) ? sec.bars : [{text:""}];

  return bars.map((bar, bi)=>{
    const beats = computeBeats(String(bar?.text || ""));
    // If everything is empty, keep one nbsp so line height remains stable
    const allEmpty = beats.every(x => !String(x||"").length);
    const safe = allEmpty ? ["\u00A0","","",""] : beats;

    const spans = safe.map((t, beatIdx)=>{
      const html = escapeHtml(String(t ?? ""));
      const content = html.length ? html : "\u00A0";
      return `<span class="fullBeatSpan" data-sec-key="${escAttr(key)}" data-bar-idx="${bi}" data-beat-idx="${beatIdx}">${content}</span>`;
    }).join("");

    return `<div class="fullOverlayLine bar" data-sec-key="${escAttr(key)}" data-bar-idx="${bi}">${spans}</div>`;
  }).join("");
}

function renderFullSongFlow(p, mount, opts={}){
  mount.innerHTML = "";
  mount.className = "fullSongFlow";

  const highlightMode = !!(opts && opts.highlightMode);

  // mark mode so other helpers can detect it
  mount.dataset.fullMode = (opts && opts.highlightMode) ? "highlight" : "edit";
  ...
  for(const key of keys){
    ...
    block.appendChild(h{
      ...
      btnRow.appendChild(delBtn);
            ...
            // Wrap editor so we can place a highlight overlay on top during AutoScroll (FULL view)
      const editorWrap = document.createElement("div");
      editorWrap.className = "fullEditorWrap";
      editorWrap.dataset.secKey = key;

      // Always keep the normal Full editor (textarea) in the DOM
      editorWrap.appendChild(body);

      // If AutoScroll is ON while viewing FULL, we render a transparent overlay that can flash per-beat text segments
      if(opts && opts.highlightMode){
        ensureFullOverlayStyles();
        body.readOnly = true;
        body.classList.add("fullHiddenText");

        const overlay = document.createElement("div");
        overlay.className = "fullHighlightOverlay";
        overlay.dataset.secKey = key;
        overlay.innerHTML = buildFullHighlightHTML(p, key);
        editorWrap.appendChild(overlay);
      }else{
        body.readOnly = false;
        body.classList.remove("fullHiddenText");
      }
...
function renderSectionBarsInto(p, sectionKey, mountEl){
  ...
  sec.bars.forEach((bar, idx)=>{
    ...
    ta.addEventListener("keydown", (e)=>{
      if(e.key === "Enter"){
        e.preventDefault();
        const next = mountEl.querySelector(`textarea[data-sec="${escSel(sectionKey)}"][data-idx="${idx+1}"]`);
        if(next) next.focus();
      }
    });
...
document.addEventListener("click", (e)=>{
  ...
  if(action === "addBarAfter"){
    ...
    requestAnimationFrame(()=>{
      ...
      const newIdx = idx + 1;
      page?.querySelector(`textarea[data-sec="${escSel(secKey)}"][data-idx="${newIdx}"]`)?.focus?.();
    });
...
if(fullMount){
  ...
  const wantFullPlayMode = !!(autoScrollOn && ((p && p.activeSection === "full") || (getVisibleRealPageKeyFromPager(p) === "full")));
  renderFullSongFlow(p, fullMount, { highlightMode: wantFullPlayMode });
