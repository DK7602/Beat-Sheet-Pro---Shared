...
function getNotesTypingFontSize(){
  try{
    return String(getActiveProject?.()?.notesTypingFontSize || els.notesFontSize?.value || "3");
  }catch(_e){
    return "3";
  }
}
...
function applyNotesTypingFontSize(size, persist=false){
  try{
    const s = String(size || "3");
    if(els.notesFontSize) els.notesFontSize.value = s;
    const p = getActiveProject?.();
    if(persist && p && String(p.notesTypingFontSize || "3") !== s){
      p.notesTypingFontSize = s;
      touchProject?.(p);
    }
  }catch(_e){}
}
...
function getNotesSelectionRange(){
  try{
    const ed = els.notesEditor;
    if(!ed) return null;
...
function primeNotesTypingSpan(styleObj={}){
  try{
    const ed = els.notesEditor;
    if(!ed) return;

    ed.focus();
    const range = getNotesSelectionRange();
    ...
    span.setAttribute("data-notes-typing-style", "1");
    if(styleObj && styleObj.color) span.setAttribute("data-notes-typing-color", styleObj.color);
...
function primeNotesTypingColor(color){
  primeNotesTypingSpan({ color: String(color || "#111111") });
}

function normalizeNotesFontSizes(root){
  try{
    if(!root) return;
    const sizeMap = { "1":"12px", "2":"14px", "3":"18px", "4":"22px", "5":"28px", "6":"36px", "7":"48px" };
    root.querySelectorAll?.('font[size]').forEach((node)=>{
      const span = document.createElement('span');
      const px = sizeMap[String(node.getAttribute('size') || '3')] || '18px';
...
function createNotesPlainLine(initialHtml="<br>"){
  const line = document.createElement("div");
  line.className = "notesPlainLine";
  line.innerHTML = initialHtml || "<br>";
  return line;
}

function createNotesCheckRow(textHtml="", checked=false){
  const row = document.createElement("div");
  row.className = "notesCheckRow" + (checked ? " isChecked" : "");
  row.setAttribute("data-notes-check-row", "1");

  const box = document.createElement("input");
  box.type = "checkbox";
  box.className = "notesCheckBox";
  box.checked = !!checked;
  box.setAttribute("aria-label", "Checklist item");
  box.setAttribute("contenteditable", "false");

  const text = document.createElement("div");
  text.className = "notesCheckText";
  text.setAttribute("contenteditable", "true");
  text.spellcheck = true;
  text.innerHTML = textHtml || "<br>";
...
function focusNotesNodeStart(node){
  try{
    if(!node) return;
    const sel = window.getSelection?.();
    if(!sel) return;
...
function focusNotesNodeEnd(node){
  try{
    if(!node) return;
    const sel = window.getSelection?.();
    if(!sel) return;
...
function getActiveNotesCheckText(){
  const sel = window.getSelection?.();
  let node = sel && sel.rangeCount ? sel.getRangeAt(0).startContainer : null;
  if(node && node.nodeType === Node.TEXT_NODE) node = node.parentNode;
  return node && node.closest ? node.closest('.notesCheckText') : null;
}

function insertNotesChecklistRow(){
  try{
    const ed = els.notesEditor;
    ...
    const activeText = getActiveNotesCheckText();
    if(activeText){
      const row = activeText.closest('.notesCheckRow');
      const newRow = createNotesCheckRow('', false);
      row?.parentNode?.insertBefore(newRow, row.nextSibling);
      const newText = newRow.querySelector('.notesCheckText');
      newText?.focus();
      focusNotesNodeStart(newText);
    }else{
      let sel = window.getSelection?.();
      if(!sel || !sel.rangeCount){
        placeCaretAtEnd(ed);
      ...
      const row = createNotesCheckRow('', false);
      ...
      if(row.nextSibling){
        ...
        row.parentNode.appendChild(after);
      }
      const text = row.querySelector('.notesCheckText');
      text?.focus();
      focusNotesNodeStart(text);
    }
...
function convertNotesCheckRowToPlain(row, placeCaret=true){
  try{
    if(!row || !row.parentNode) return null;
    const text = row.querySelector('.notesCheckText');
    const plain = createNotesPlainLine(text ? text.innerHTML : '<br>');
    row.parentNode.replaceChild(plain, row);
    if(placeCaret){
      plain.focus?.();
      focusNotesNodeStart(plain);
    }
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
    return plain;
...
function isCaretAtStartOfNode(node){
  try{
    const sel = window.getSelection?.();
...
function noteExec(cmd, val=null){
  ...
  if(cmd === "fontSize"){
    const size = String(val || "3");
    const pxMap = { "1":"12px", "2":"14px", "3":"18px", "4":"22px", "5":"28px", "6":"36px", "7":"48px" };
    if(isCollapsed){
      primeNotesTypingSpan({ fontSize: pxMap[size] || pxMap["3"] });
    }else{
      try{ document.execCommand("fontSize", false, size); }catch(_e){}
      normalizeNotesFontSizes(ed);
    }
    applyNotesTypingFontSize(size, true);
    saveNotesSelection();
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
    return;
...
function normalizeNotesHtml(html){
  const cleaned = String(html || "")
    .replace(/​/g, "")
    .replace(/<span([^>]*)data-notes-typing-(?:color|style)=("[^"]*"|'[^']*')([^>]*)><\/span>/gi, "")
    .replace(/<span([^>]*)data-notes-typing-(?:color|style)=("[^"]*"|'[^']*')([^>]*)>\s*<br\s*\/?\s*>\s*<\/span>/gi, "<br>");
  const t = cleaned.trim();
  return t === "<br>" ? "" : t;
}

function saveNotesFromEditor(force=false){
  const p = getActiveProject?.();
  const ed = els.notesEditor;
  if(!p || !ed) return;
  const html = normalizeNotesHtml(ed.innerHTML);
  if(!force && (p.notesHtml || "") === html) return;
  p.notesHtml = html;
...
let notesMutationObserver = null;
function queueNotesAutosave(delay=0){
  try{
    clearTimeout(notesAutosaveTimer);
    notesAutosaveTimer = setTimeout(()=>{
      saveNotesFromEditor(true);
    }, Math.max(0, Number(delay || 0)));
  }catch(_e){}
}

function ensureNotesMutationObserver(){
  try{
    const ed = els.notesEditor;
    if(!ed || notesMutationObserver) return;
    notesMutationObserver = new MutationObserver(()=>{
      if(!notesOpen) return;
      queueNotesAutosave(60);
    });
    notesMutationObserver.observe(ed, { childList:true, subtree:true, characterData:true, attributes:true });
  }catch(_e){}
}
...
function resetNotesTypingState(){
  try{
    ...
    const color = getNotesTypingColor();
    const size = getNotesTypingFontSize();
    const pxMap = { "1":"12px", "2":"14px", "3":"18px", "4":"22px", "5":"28px", "6":"36px", "7":"48px" };
    applyNotesTypingColor(color, false);
    applyNotesTypingFontSize(size, false);

    // When a saved note is reopened, browsers often fall back to inherited black
    // text for a collapsed caret position. Prime the caret with the saved typing
    // style so new text keeps using the currently selected options after refresh.
    const sel = window.getSelection?.();
    const hasCollapsedCaret = !!(sel && sel.rangeCount && sel.getRangeAt(0).collapsed);
    if(hasCollapsedCaret){
      primeNotesTypingSpan({ color, fontSize: pxMap[size] || pxMap["3"] });
      saveNotesSelection();
    }
  }catch(_e){}
}
...
function openNotes(){
  ...
  ensureNotesMutationObserver();
  queueNotesAutosave(0);
  setDockHidden(true);
  updateRhymes("");
...
function wireNotesEditor(){
  ...
  ed.addEventListener("change", (e)=>{
    const box = e.target?.closest?.('.notesCheckBox');
    if(!box) return;
    const row = box.closest('.notesCheckRow');
    if(row) row.classList.toggle('isChecked', !!box.checked);
    saveNotesSelection();
    saveNotesFromEditor();
    scheduleNotesRhymeRefresh();
  });
  ...
  ed.addEventListener("keydown", (e)=>{
    const checkText = e.target?.closest?.('.notesCheckText') || getActiveNotesCheckText();
    if(!checkText) return;
    const row = checkText.closest('.notesCheckRow');
    if(!row) return;

    if(e.key === 'Enter'){
      e.preventDefault();
      const isEmpty = !String(checkText.innerText || '').replace(/​/g,'').trim();
      if(isEmpty){
        convertNotesCheckRowToPlain(row, true);
      }else{
        const next = createNotesCheckRow('', false);
        row.parentNode.insertBefore(next, row.nextSibling);
        const nextText = next.querySelector('.notesCheckText');
        nextText?.focus();
        focusNotesNodeStart(nextText);
        saveNotesFromEditor();
        scheduleNotesRhymeRefresh();
      }
      return;
    ...
    if(e.key === 'Backspace'){
      const isEmpty = !String(checkText.innerText || '').replace(/​/g,'').trim();
      if(isEmpty && isCaretAtStartOfNode(checkText)){
        e.preventDefault();
        convertNotesCheckRowToPlain(row, true);
      }
      return;
  ...
  ed.addEventListener("input", ()=>{
    ...
    applyNotesTypingFontSize(getNotesTypingFontSize(), false);
    if(normalizeNotesHtml(ed.innerHTML)){
      ed.style.removeProperty("color");
    }
    saveNotesSelection();
    queueNotesAutosave(40);
    scheduleNotesRhymeRefresh();
  });
  ed.addEventListener("compositionend", ()=>{ saveNotesSelection(); queueNotesAutosave(0); scheduleNotesRhymeRefresh(); });
  ed.addEventListener("keyup", ()=>{ saveNotesSelection(); queueNotesAutosave(0); scheduleNotesRhymeRefresh(); });
  ...
  ed.addEventListener("blur", ()=>{ queueNotesAutosave(0); });
  ed.addEventListener("focus", ()=>{ applyNotesTypingColor(getNotesTypingColor(), false); applyNotesTypingFontSize(getNotesTypingFontSize(), false); saveNotesSelection(); scheduleNotesRhymeRefresh(); });
...
function repairProject(p){
  ...
  if(typeof p.notesTypingFontSize !== "string" || !p.notesTypingFontSize) p.notesTypingFontSize = "3";
...
els.notesChecklistBtn?.addEventListener("click", ()=> insertNotesChecklistRow());
...
[els.notesBoldBtn, els.notesUnderlineBtn, els.notesItalicBtn, els.notesStrikeBtn, els.notesChecklistBtn, els.notesUndoBtn, els.notesRedoBtn, els.notesFontSize, els.notesColor].forEach((el)=>{
  ["pointerdown","mousedown","touchstart"].forEach((evt)=>{
    el?.addEventListener(evt, ()=>{ if(notesOpen) saveNotesSelection(); }, { passive:true });
  });
});

document.addEventListener("visibilitychange", ()=>{
  if(document.visibilityState !== "hidden" || !notesOpen) return;
  saveNotesFromEditor(true);
});
window.addEventListener("pagehide", ()=>{ if(notesOpen) saveNotesFromEditor(true); });
window.addEventListener("beforeunload", ()=>{ if(notesOpen) saveNotesFromEditor(true); });
