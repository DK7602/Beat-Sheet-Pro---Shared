...
function setAutoScroll(v){
  ...
  if(!autoScrollOn){
    ...
    if(p){
      try{ setFullHighlightModeEnabled(p, false); }catch(_e){}
  ...
  if(turningOn){
    ...
    if(p){
      ...
      if(visibleKey === "full"){
        // ✅ Full Song View: enable overlay so we can detect what's in view, then anchor to the first visible bar line
        try{ setFullHighlightModeEnabled(p, true); }catch(_e){}
        const a = getFirstVisibleFullAnchor();
        const secKey = a?.secKey || (getPlaySequence(p)[0]?.secKey) || (getActiveSectionKeysForFullView(p)[0]) || null;
        const barIdx = Number.isFinite(a?.barIdx) ? a.barIdx : 0;
        p.playback.seqStartOffset = computeSeqStartOffsetFromAnchor(p, secKey, barIdx);
      }else{
...
function startPracticeScroll(p){
  ...
  if(activeProject){
    ...
    if(visibleKey === "full"){
      try{ setFullHighlightModeEnabled(activeProject, true); }catch(_e){}
      const a = getFirstVisibleFullAnchor();
      const secKey = a?.secKey || (getPlaySequence(activeProject)[0]?.secKey) || (getActiveSectionKeysForFullView(activeProject)[0]) || null;
      const barIdx = Number.isFinite(a?.barIdx) ? a.barIdx : 0;
      activeProject.playback.seqStartOffset = computeSeqStartOffsetFromAnchor(activeProject, secKey, barIdx);
    }else{
      const firstVisibleBarIdx = getFirstVisibleBarIdxInActivePage();
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  // ✅ FULL page participates: keep user on Full Song View, but highlight/scroll using the SAME play-sequence.
  // (No horizontal page-jumping.)
  const onFull = (pageKey === "full");
  if(!onFull){ try{ setFullHighlightModeEnabled(p, false); }catch(_e){} }

  if(onFull && !autoScrollOn){
    // When AutoScroll is OFF, Full view doesn't flash (there are no beat boxes there).
    return;
  }

  // Ensure Full highlight overlay is enabled while autoscrolling on Full view
  if(onFull && autoScrollOn){
    try{ setFullHighlightModeEnabled(p, true); }catch(_e){}
  }
  ...
  if(onFull){
    const lineEl = flashFullOverlayBeat(p, target.secKey, target.barIdx, safeBeat);

    if(lineEl && safeBeat === 0){
      const token = `full:${target.secKey}:${target.barIdx}`;
      if(token !== lastAutoScrollToken){
        lastAutoScrollToken = token;
        requestAnimationFrame(()=>scrollBarIntoView(lineEl));
      }
    }
    return;
  }
...
function ensureFullHighlightStyles(){
  if(document.getElementById("fullHighlightStyles")) return;
  const st = document.createElement("style");
  st.id = "fullHighlightStyles";
  st.textContent = `
  .fullEditorWrap{ position:relative; width:100%; }
  ...
  .fullHighlightOverlay.hidden{ display:none; }
  .fullBeatSpan{ border-radius:10px; padding:1px 4px; }
  .fullBeatSpan.flash{
    outline:3px solid #facc15;
    box-shadow: 0 0 0 4px rgba(250,204,21,0.55);
    background: rgba(250,204,21,0.65);
  }
  .fullBarLine{ display:block; }
  .fullBarGap{ display:block; height:12px; }
  `;
  document.head.appendChild(st);
}

function buildFullOverlayHTMLForSection(p, secKey){
  const sec = p?.sections?.[secKey];
  const bars = sec?.bars || [];
  let out = "";
  for(let i=0;i<bars.length;i++){
    const raw = (bars[i]?.text ?? "");
    const normalized = String(raw).replace(/\s+/g," ").trim();
    const beats = computeBeats(normalized);

    out += `<div class="fullBarLine" data-sec="${escAttr(secKey)}" data-bar-idx="${i}">`;
    for(let b=0;b<4;b++){
      const chunk = beats[b] || "";
      const spacer = (b > 0) ? " " : "";
      out += `${spacer}<span class="fullBeatSpan" data-beat="${b}">${escapeHtml(chunk)}</span>`;
    }
    out += `</div>`;
    if(i < bars.length - 1) out += `<div class="fullBarGap"></div>`;
  }
  return out;
}

function syncOverlayBoxToTextarea(textarea, overlay){
...
function setFullHighlightModeEnabled(p, enabled){
  try{
    const pageEl = getVisiblePagerPageEl();
    if(!pageEl) return;
    if((pageEl.dataset?.pageKey || "") !== "full") return;

    ensureFullHighlightStyles();

    const wraps = pageEl.querySelectorAll(".fullEditorWrap");
    wraps.forEach(w=>{
      const ta = w.querySelector("textarea.fullSectionEditor");
      const ov = w.querySelector(".fullHighlightOverlay");
      if(!ta || !ov) return;

      if(enabled){
        ov.classList.remove("hidden");
        const secKey = w.getAttribute("data-sec-key") || ta.dataset.secEditor;
        ov.innerHTML = buildFullOverlayHTMLForSection(p, secKey);
        syncOverlayBoxToTextarea(ta, ov);

        ta.readOnly = true;
        ta.style.color = "transparent";
        ta.style.caretColor = "transparent";
        ta.style.textShadow = "0 0 0 rgba(0,0,0,0)";
      }else{
...
function getVisiblePagerPageEl(){
  try{
    const pagerEl = document.getElementById("pagesPager") || (els?.bars?.querySelector?.(".pager"));
...
function getFirstVisibleFullAnchor(){
  try{
    const pageEl = getVisiblePagerPageEl();
    if(!pageEl || (pageEl.dataset?.pageKey || "") !== "full") return null;

    const scroller = findVerticalScroller(pageEl) || document.scrollingElement;
    const isInner = (scroller && scroller !== document.body && scroller !== document.documentElement && scroller !== document.scrollingElement);
    const vr = isInner ? scroller.getBoundingClientRect() : { top: 0, bottom: window.innerHeight };
    const viewTop = vr.top;

    const lines = Array.from(pageEl.querySelectorAll(".fullHighlightOverlay:not(.hidden) .fullBarLine[data-sec][data-bar-idx]"));
    if(!lines.length) return null;

    for(const el of lines){
      const r = el.getBoundingClientRect();
      if(r.bottom > viewTop + 8){
        const secKey = el.getAttribute("data-sec") || "";
        const barIdx = parseInt(el.getAttribute("data-bar-idx") || "0", 10);
        if(secKey) return { secKey, barIdx: Number.isFinite(barIdx) ? barIdx : 0 };
        return null;
      }
    }
    const el0 = lines[0];
    return { secKey: el0.getAttribute("data-sec") || "", barIdx: parseInt(el0.getAttribute("data-bar-idx")||"0",10) || 0 };
  }catch(_e){
    return null;
  }
}

function flashFullOverlayBeat(p, secKey, barIdx, beatIdx){
  try{
    const pageEl = getVisiblePagerPageEl();
    if(!pageEl || (pageEl.dataset?.pageKey || "") !== "full") return null;

    pageEl.querySelectorAll(".fullBeatSpan.flash").forEach(el=>el.classList.remove("flash"));

    const esc = (window.CSS && CSS.escape) ? CSS.escape : (s)=>String(s).replace(/[^a-zA-Z0-9_\-]/g,'\\$&');
    const line = pageEl.querySelector(`.fullBarLine[data-sec="${esc(secKey)}"][data-bar-idx="${barIdx}"]`);
    if(!line) return null;

    const span = line.querySelector(`.fullBeatSpan[data-beat="${beatIdx}"]`);
    if(span){
      span.classList.add("flash");
      setTimeout(()=>{ try{ span.classList.remove("flash"); }catch(_e){} }, 90);
    }
    return line;
  }catch(_e){
    return null;
...
function renderFullSongFlow(p, mount){
  ...
  for(const key of keys){
    ...
    wrap.className = "fullEditorWrap";
    wrap.setAttribute("data-sec-key", key);

    const overlay = document.createElement("div");
    overlay.className = "fullHighlightOverlay hidden";
    overlay.setAttribute("data-sec-key", key);
  ...
  try{
    const visible = getVisibleRealPageKeyFromPager(p) || p.activeSection || "full";
    setFullHighlightModeEnabled(p, !!(autoScrollOn && visible === "full"));
  }catch(_e){}
}
