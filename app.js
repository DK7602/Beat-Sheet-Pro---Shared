...
function attachPullToRefresh(scrollEl, onRefresh){
  ...
  const MIN_MS = 200; // must be a deliberate pull
  ...
  const outerScrollTop = ()=>{
    // If the page itself can scroll (Chrome UI, keyboard resize, etc.),
    // treat any non-zero window scroll as NOT eligible for PTR.
    let top = 0;
    try{ top += (scrollEl.scrollTop || 0); }catch(_){ }
    try{
      const se = document.scrollingElement || document.documentElement;
      top += (se && se.scrollTop) ? se.scrollTop : 0;
    }catch(_){ }
    return top;
  };
  ...
  root.addEventListener("touchstart", (e)=>{
    ...
    // Only if the gesture starts inside this scroll container
    const tgt0 = e.target;
    if(!(tgt0 && scrollEl.contains(tgt0))) return;

    if(outerScrollTop() > 0) return;
    if(isInteractive(e.target)) return;

    // If the gesture starts inside an inner scrollable window, only allow PTR when that inner window is at the top.
    startInnerScroll = findInnerScrollParent(e.target);
    if(startInnerScroll && (startInnerScroll.scrollTop || 0) > 0) return;

    // Allow pull-to-refresh even if a textarea is focused, as long as
    // the scrollable window you're pulling in is already at the top.
  ...
  root.addEventListener("touchmove", (e)=>{
    ...
    // If finger moves outside the container, cancel
    const tgt = e.target;
    if(!(tgt && scrollEl.contains(tgt))){ tracking = false; return; }

    if(outerScrollTop() > 0){ tracking = false; return; }
    if(startInnerScroll && (startInnerScroll.scrollTop || 0) > 0){ tracking = false; return; }
    ...
    // Don't cancel tracking on tiny upward jitters; just wait for a real downward pull.
    if(dy <= 0) return;
