...
function syncHeaderHeightVar(){
  ...
  const measure = ()=>{
    ...
    if(!collapsed){
      ...
      // â Make the header itself extend exactly to the bottom of the drum row
...
/***********************
â remembers last textarea user typed in (mobile fix)
***********************/
...
/***********************
â textarea auto-grow (mobile scroll fix)
...
function setAutoScroll(v){
  ...
  if(!autoScrollOn){
    ...
    try{ const fullPageEl = getVisiblePageElFromPagerByKey('full'); if(fullPageEl) setFullOverlayMode(fullPageEl, false); }catch(e){}
  ...
  if(turningOn){
    ...
    if(p){
      ...
      // â If user enables Scroll while viewing FULL, start from the first FULL line in view.
      if(visibleKey === "full"){
        p.playback.seqStartOffset = computeSeqStartOffsetFromFullViewport(p);
      }else{
...
// â cache for decoded audio buffers (prevents re-decode lag)
...
/***********************
â Keep rhyme dock visible above keyboard (Android)
***********************/
...
/***********************
â syllables (improved) + beat splitting (NO word splitting)
***********************/
...
function autoSplitSyllablesClean(text){
  ...
  for(let i=0;i<words.length;i++){
    ...
    // â if it would overshoot AND we already have something in this beat AND we still have next beats,
...
// Offline fallback: pull near-rhymes from words already in the song
function localRhymes(seed){
  try{
    const w = String(seed||"" ).toLowerCase();
    if(!w) return [];
    const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
    if(!p || !p.sections) return [];
    const suff3 = w.slice(-3);
    const suff2 = w.slice(-2);
    ...
    const push = (x)=>{
      ...
      if(suff3 && x.endsWith(suff3)) { seen.add(x); out.push(x); return; }
      if(suff2 && x.endsWith(suff2)) { seen.add(x); out.push(x); }
    };
    for(const k of Object.keys(p.sections)){
      const sec = p.sections[k];
      const bars = (sec && sec.bars) ? sec.bars : [];
      for(const b of bars){
        const txt = String((b && b.text) ? b.text : "");
        const words = txt.toLowerCase().replace(/[^a-z0-9's-]/g," ").split(/s+/).filter(Boolean);
        for(const ww of words) push(ww);
...
async function updateRhymes(seed){
  ...
  if(!w){
    if(els.rhymeBase) els.rhymeBase.textContent = "Tap into a beatâ¦";
  ...
  if(els.rhymeList) els.rhymeList.innerHTML = `<span class="small">Loadingâ¦</span>`;
...
/***********************
â rhymes: beat-box tap delegation (guaranteed)
***********************/
document.addEventListener("click", (e)=>{
  const beat = e.target.closest(".beat");
  if(!beat) return;
  if(beat.closest(".rhymeDock")) return;

  const beatsWrap = beat.parentElement;
  if(!beatsWrap || !beatsWrap.classList || !beatsWrap.classList.contains("beats")) return;

  const barEl = beat.closest(".bar");
  if(!barEl) return;

  const pageEl = beat.closest(".page");
  const secKey = pageEl && pageEl.dataset ? pageEl.dataset.pageKey : null;
  if(!secKey || secKey === "full") return;

  const ta = barEl.querySelector("textarea");
  if(!ta) return;

  const beatEls = Array.from(beatsWrap.children || []).filter(el=>el && el.classList && el.classList.contains("beat"));
  const bi = beatEls.indexOf(beat);

  try{
    const b = computeBeats(ta.value || "");
    let prevText = "";
    if(bi > 0){
      prevText = b[bi-1] || "";
    }else{
      const idx = parseInt(barEl.getAttribute("data-bar-idx") || (barEl.dataset ? barEl.dataset.barIdx : "0") || "0", 10) || 0;
      const p = getActiveProject();
      const prevBar = p?.sections?.[secKey]?.bars?.[idx-1];
...
function repairProject(p){
  ...
  // â ensure base sections exist
  for(const def of BASE_SECTION_DEFS){
    if(!p.sections[def.key] || typeof p.sections[def.key] !== "object"){
      // â base sections exist but titles stay user-editable (no auto-fill)
  ...
  // â ensure extras listed exist as sections
  ...
  // â if active section no longer exists as a page, snap to FULL
...
// â alias used by some UI actions (keeps older call sites safe)
function saveProject(p){
  try{ touchProject(p); }catch(e){}
}

// â Ensure legacy/global call sites never throw a ReferenceError
// (some shared builds still call saveProject from inline handlers).
try{ window.saveProject = saveProject; }catch(_e){}


/***********************
â migrate old audio (dataUrl -> idb)
***********************/
...
let recordDest = null;
let recordMix = null;      // â sums mic + drums + playback for recording
let recordLimiter = null;  // â prevents recording breakup

let metroTimer = null;
let metroBeat16 = 0;
let autoScrollMetroOffset16 = 0;

// â practice-mode autoscroll (no audio, no drums)
...
function getFirstVisibleBarIdxInActivePage(){
  // â Choose the bar/card with the MOST visible area in the current viewport.
...
function ensureAudio(){
  if(!audioCtx){
    ...
    // â MASTER SPEAKER BUS (prevents playback breakup/clipping)
...
playbackGain.connect(masterMix);
    ...
    // â drums go to recording mix (not straight to recordDest)
    ...
    // â playback can be included in recordings if you want
...
/***********************
â pager helpers (clone-safe) â used by FULL autoscroll + overlay
***********************/
function getPagerEl(){
  return document.getElementById("pagesPager") || document.querySelector(".pager");
}

function getVisiblePageElFromPager(){
  const pager = getPagerEl();
  if(!pager) return null;
  const pages = Array.from(pager.querySelectorAll(".page"));
  if(!pages.length) return null;

  const viewportLeft = pager.scrollLeft;
  const viewportCenter = viewportLeft + (pager.clientWidth / 2);

  let best = null;
  let bestDist = Infinity;
  for(const pg of pages){
    const left = pg.offsetLeft;
    const center = left + (pg.offsetWidth / 2);
...
/**
 * Returns the page element for a given pageKey that is MOST visible in the pager viewport.
 * Works with clones (data-clone="1") too.
 */
function getVisiblePageElFromPagerByKey(pageKey){
  const pager = getPagerEl();
  if(!pager) return null;
  const esc = (window.CSS && CSS.escape) ? CSS.escape : (s)=>String(s).replace(/[^a-zA-Z0-9_\-]/g,'\\$&');

  const pages = Array.from(pager.querySelectorAll(`.page[data-page-key="${esc(pageKey)}"]`));
  if(!pages.length) return null;
  ...
  let bestDist = Infinity;
  for(const pg of pages){
    const center = pg.offsetLeft + (pg.offsetWidth / 2);
    const d = Math.abs(center - viewportCenter);
    if(d < bestDist){
      bestDist = d;
      best = pg;
...
/*************
â NEW: build "play sequence" of ONLY text cards across all sections
*************/
...
// â Determine the real (non-clone) page key currently visible in the horizontal pager.
...
// â Convert an (anchor page key + bar idx) to the global play-sequence offset.
...
// Inject CSS for full-view beat flashing (needed because FULL uses span overlay)
let _fullFlashCssInjected = false;


/***********************
â FULL VIEW OVERLAY â ensure helper (was missing)
***********************/
function ensureFullOverlays(p, fullPageEl){
  try{
    if(!fullPageEl) return;
    // Build/update overlay DOM + text
    ensureFullSectionOverlays(fullPageEl);
    // Turn it on visually
    setFullOverlayMode(fullPageEl, true);
  }catch(e){}
}

function ensureFullFlashCss(){
  if(document.getElementById("bspFullFlashCss")) return;
  const css = document.createElement("style");
  css.id = "bspFullFlashCss";
  css.textContent = `
  /* Full Song View beat-highlight overlay (textarea overlay) */
  .fullBodyWrap{ position:relative; }

  .fullBodyWrap .fullOverlayWrap{
    position:absolute;
    inset:0;
    pointer-events:none;
    z-index:5;
    ...
    overflow:hidden; /* clip to textarea viewport */
  }
  .fullOverlayWrap .fullOverlayText{
    position:relative;
    transform: translateY(0px); /* synced to textarea.scrollTop */
  }

  .fullOverlayLine{ display:block; }
  .fullBeat{ display:inline; border-radius:8px; padding:0 2px; }
  .fullBeat.flash{
    background: rgba(255, 230, 0, 0.55);
    box-shadow: 0 0 0 2px rgba(255, 230, 0, 0.95) inset;
  }
  `;
  document.head.appendChild(css);
}

function getVisiblePagerKey(){
  try{
    const pagerEl = document.getElementById("pagesPager");
    if(!pagerEl) return null;
    const idx = getCurrentIdx(pagerEl);
    const cur = pagerEl.children && pagerEl.children[idx] ? pagerEl.children[idx] : null;
    return (cur && cur.dataset) ? (cur.dataset.pageKey || null) : null;
  }catch(e){
    return null;
  }
}

function ensureFullSectionOverlays(fullPageEl){
  ensureFullFlashCss();

  const sections = Array.from(fullPageEl.querySelectorAll(".fullSection"));
  for(const secEl of sections){
    const ta = secEl.querySelector("textarea.fullSectionBody");
    if(!ta) continue;

    // Ensure a dedicated wrap around the textarea so the overlay only covers the text area (not headers/pills)
    let bodyWrap = secEl.querySelector(".fullBodyWrap");
    if(!bodyWrap){
      bodyWrap = document.createElement("div");
      bodyWrap.className = "fullBodyWrap";
      // insert wrap right before textarea, then move textarea into it
      ta.parentNode.insertBefore(bodyWrap, ta);
      bodyWrap.appendChild(ta);
    }

    let overlay = bodyWrap.querySelector(".fullOverlayWrap");
    if(!overlay){
      overlay = document.createElement("div");
      overlay.className = "fullOverlayWrap";
      overlay.innerHTML = `<div class="fullOverlayText"></div>`;
      bodyWrap.appendChild(overlay);
    }

    const overlayText = overlay.querySelector(".fullOverlayText");

    // Match textarea box model for pixel-perfect overlay
    ...
    // Keep overlay viewport height synced with textarea viewport height
    overlay.style.height = ta.clientHeight + "px";

    // Build overlay text from the project's current bars (source of truth)
    const secKey = secEl.dataset.secKey || ta.dataset.secEditor;
    const p = getActiveProject();
    const sec = p && p.sections ? p.sections[secKey] : null;
    const bars = (sec && sec.bars) ? sec.bars : [];
    const html = bars.map((b, bi)=>{
      const beats = computeBeats((b && b.text) ? b.text : "");
      return `<span class="fullOverlayLine" data-sec="${escapeHtml(secKey)}" data-bar="${bi}">` +
        `<span class="fullBeat" data-beat="0">${escapeHtml(beats[0]||"")}</span> ` +
        `<span class="fullBeat" data-beat="1">${escapeHtml(beats[1]||"")}</span> ` +
        `<span class="fullBeat" data-beat="2">${escapeHtml(beats[2]||"")}</span> ` +
        `<span class="fullBeat" data-beat="3">${escapeHtml(beats[3]||"")}</span>` +
      `</span>`;
    }).join("\n");
    if(overlayText) overlayText.innerHTML = html;

    // Sync overlay scroll position to textarea scrollTop (move overlayText up as textarea scrolls)
    if(!ta._bspFullOverlayScrollBound){
      ta._bspFullOverlayScrollBound = true;
      ta.addEventListener("scroll", ()=>{
        try{
          const wrap = ta.closest(".fullBodyWrap");
          const ov = wrap && wrap.querySelector(".fullOverlayWrap .fullOverlayText");
          if(!ov) return;
    ...
    // initial sync
    try{
      if(overlayText) overlayText.style.transform = `translateY(${-ta.scrollTop}px)`;
    }catch(e){}
  }
}

function setFullOverlayModefunction setFullOverlayMode(fullPageEl, on){
  try{
    const sections = Array.from(fullPageEl.querySelectorAll(".fullSection"));
    for(const secEl of sections){
      const ta = secEl.querySelector("textarea.fullSectionBody");
      if(on){
        secEl.classList.add("fullOverlayOn");
        if(ta){ ta.style.color = "transparent"; ta.style.caretColor = "transparent"; }
        const overlay = secEl.querySelector(".fullOverlayWrap");
        if(overlay) overlay.style.display = "block";
      }else{
        secEl.classList.remove("fullOverlayOn");
...
let lastFullFlashToken = null;
function flashFullBeat(fullPageEl, secKey, barIdx, beatIdx){
  try{
    ensureFullSectionOverlays(fullPageEl);
    setFullOverlayMode(fullPageEl, true);

    const token = `${secKey}:${barIdx}:${beatIdx}`;
    if(token === lastFullFlashToken) return;
    lastFullFlashToken = token;

    // clear previous flash
    fullPageEl.querySelectorAll(".fullBeat.flash").forEach(el=>el.classList.remove("flash"));

    const esc = (window.CSS && CSS.escape) ? CSS.escape : (s)=>String(s).replace(/[^a-zA-Z0-9_\-]/g,'\\$&');
    const line = fullPageEl.querySelector(`.fullOverlayLine[data-sec="${esc(secKey)}"][data-bar="${barIdx}"]`);
    if(!line) return;
    const beat = line.querySelector(`.fullBeat[data-beat="${beatIdx}"]`);
    if(!beat) return;
    beat.classList.add("flash");

    // flash off quickly to mimic BSP beat flash
    setTimeout(()=>{ try{ beat.classList.remove("flash"); }catch(e){} }, 140);
  }catch(e){}
}

function scrollFullLineIntoView(fullPageEl, secKey, barIdx){
  try{
    const esc = (window.CSS && CSS.escape) ? CSS.escape : (s)=>String(s).replace(/[^a-zA-Z0-9_\-]/g,'\$&');
    const line = fullPageEl.querySelector(`.fullOverlayLine[data-sec="${esc(secKey)}"][data-bar="${barIdx}"]`);
    if(!line) return;

    // Find the corresponding textarea viewport and scroll it to bring this line into view
    const secEl = line.closest(".fullSection");
    const ta = secEl && secEl.querySelector("textarea.fullSectionBody");
    if(!ta) return;

    const targetTop = line.offsetTop; // offset within overlayText (untransformed)
    const desired = Math.max(0, targetTop - Math.floor(ta.clientHeight * 0.35));
    ta.scrollTop = desired;

    // keep overlayText synced immediately
    const wrap = ta.closest(".fullBodyWrap");
    const ov = wrap && wrap.querySelector(".fullOverlayWrap .fullOverlayText");
    if(ov) ov.style.transform = `translateY(${-ta.scrollTop}px)`;
  }catch(e){}
}

function computeSeqStartOffsetFromFullViewport(p){
  try{
    const fullPageEl = getVisiblePageElFromPagerByKey("full");
    if(!fullPageEl) return 0;
    ensureFullSectionOverlays(fullPageEl);

    const lines = Array.from(fullPageEl.querySelectorAll(".fullOverlayLine"));
    if(!lines.length) return 0;

    // choose the first line whose top is within the viewport
    const top = 140;
    let chosen = lines[0];
    ...
    const secKey = chosen.getAttribute("data-sec") || "";
    const barIdx = parseInt(chosen.getAttribute("data-bar") || "0", 10) || 0;

    const seq = getPlaySequence(p);
    if(!seq.length) return 0;
    const idx = seq.findIndex(x => x.secKey === secKey && x.barIdx === barIdx);
    return (idx >= 0) ? idx : 0;
  }catch(e){
    return 0;
  }
...
/*************
â highlight driver (now supports global text sequence)
*************/
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  // â If the user is currently viewing FULL, drive the FULL overlay/highlight even if p.activeSection is stale
  const visibleKey = getVisiblePagerKey();
  if(visibleKey === "full") pageKey = "full";

  // â FULL page participates when AutoScroll is ON (flash-highlight the matching beat text)
  if(pageKey === "full"){
    ...
    // Ensure FULL overlay exists on the VISIBLE full page (clone-safe)
    const fullPageEl = getVisiblePageElFromPagerByKey("full");
    if(!fullPageEl) return;
    ensureFullOverlays(p, fullPageEl);

    flashFullBeat(fullPageEl, target.secKey, target.barIdx, safeBeat);
    if(safeBeat === 0) scrollFullLineIntoView(fullPageEl, target.secKey, target.barIdx);
    return;
  }

  // â FIX #3: AutoScroll OFF = tick-highlight ALL cards on the active page (unchanged)
  if(!autoScrollOn){
  ...
  // â AutoScroll ON = use text-only sequence across sections
  const seq = getPlaySequence(p);
...
/***********************
â MP3 ENCODE (Auto convert WebM take -> MP3 right after recording)
...
async function encodeMp3FromFloat32Mono(samples, sampleRate){
  ...
  for(let i=0;i<samples.length;i+=blockSize){
    ...
    // yield sometimes so UI doesnât freeze on longer takes
...
async function ensureMic(){
 ...
 // â Ask for a "music-like" mic path (turn OFF call-processing)
try{
  ...
  // â after getUserMedia (after try/catch), force-disable call-processing if supported
try{
  ...
  micGain.connect(recordMix); // â goes through limiter now
}
...
/***********************
â Upload audio -> saves blob to IDB
***********************/
...
function applyFullTextToProject(p, fullText){
  ...
  // â if user typed under a deleted heading, re-enable that page
...
/***********************
â CAROUSEL PAGER (wrap) â dynamic pages
***********************/
...
function renderFullSongFlow(p, mount){
  ...
  for(const key of keys){
    ...
    pill.addEventListener("input", ()=>{
      ...
      touchProject(p);
      // â sync this title everywhere (card page title pills + any other full pills)
    ...
    addBtn.addEventListener("click", (e)=>{
      ...
      // â add the next PAGE (section) like SRP (not an extra text window)
...
// ✅ wrap textarea so overlay only covers the body (not the header pill)
...
function nextAddKeyFrom(p, fromKey){
  ...
  if(fromKey === "full"){
    ...
    for(const k of BASE_ORDER){
      ...
      if(isDeleted(p,k)) continue;         // â skip deleted until text restores
      return k;
    }
...
function renderSectionBarsInto(p, sectionKey, mountEl){
  ...
  sec.bars.forEach((bar, idx)=>{
    ...
    // â Tap a beat box to get rhymes for the PREVIOUS beat's last word
    beatEls.forEach((bEl, bi)=>{
    ...
    // Android fallback: some WebViews miss pointer events on fast taps
    beatEls.forEach((bEl, bi)=>{
      bEl.addEventListener("touchstart", (ev)=>{
        try{
          ev.preventDefault();
          ev.stopPropagation();
          ta.focus();
...
/***********************
â add/delete bar actions (delegation)
***********************/
document.addEventListener("click", (e)=>{
  ...
  if(action === "delBar"){
    ...
    if(bars.length <= 1){
      showToast("Canât delete last card");
      return;
    }
    if(Number.isNaN(idx) || idx < 0 || idx >= bars.length) return;
...
function renderRecordings(){
  ...
  ev.stopPropagation();
    ...
    playBtn.textContent = isThisPlaying ? "â¦" : "â¶";
...
function buildSplitExportText(p){
  ...
  for(const key of order){
    // skip FULL in split export (itâs the combined view)
...
els.exportBtn?.addEventListener("click", ()=>{
  ...
  const htmlA = makeHtmlDoc(`${name} â FULL`, fullText);
  downloadTextAsFile(`${name} - FULL.html`, htmlA);

  const splitText = buildSplitExportText(p).trim() || "";
  const htmlB = makeHtmlDoc(`${name} â SPLIT`, splitText);
...
function attachPullToRefresh(scrollEl, onRefresh){
  ...
  scrollEl.addEventListener("touchend", ()=>{
    if(tracking && pulled){
      try{ showToast("Refreshingâ¦"); }catch(_){}
...
/***********************
â GLOBAL BEAT TAP -> RHYMES (reliable on mobile)
***********************/
document.addEventListener("pointerdown", (ev)=>{
  try{
    const beatEl = ev.target && ev.target.closest ? ev.target.closest(".beat") : null;
    if(!beatEl) return;
    // Only handle beats inside a .beats row (cards)
    const beatsRow = beatEl.parentElement && beatEl.parentElement.classList && beatEl.parentElement.classList.contains("beats")
      ? beatEl.parentElement
      : null;
    if(!beatsRow) return;

    ev.preventDefault();

    const beatEls = Array.from(beatsRow.querySelectorAll(".beat"));
    const idx = beatEls.indexOf(beatEl);
    if(idx < 0) return;

    // Previous beat text in this bar (or last beat of previous bar)
    let prevText = "";
    if(idx > 0){
      prevText = (beatEls[idx-1].textContent || "").trim();
    }else{
      const wrap = beatsRow.closest(".barWrap") || beatsRow.closest(".bar") || beatsRow.closest(".barCard");
      const sec = wrap ? (wrap.querySelector("textarea")?.dataset?.sec || wrap.dataset?.sec || "") : "";
      const barIdx = wrap ? parseInt(wrap.querySelector("textarea")?.dataset?.idx || wrap.dataset?.idx || "0", 10) : 0;

      const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
      const s = p && p.sections ? p.sections[sec] : null;
      const prevBar = s && s.bars ? s.bars[barIdx-1] : null;
      if(prevBar && prevBar.text){
        const pb = computeBeats(prevBar.text);
        prevText = (pb[3] || pb[2] || pb[1] || pb[0] || "").trim();
      }
    }
...
// ✅ Mobile-friendly: also react to touch/pointer on beat boxes (some Android builds do not fire click reliably)
document.addEventListener("pointerdown", (e)=>{
  const beatEl = e.target && e.target.closest ? e.target.closest(".beats .beat") : null;
  if(!beatEl) return;
  const barWrap = beatEl.closest(".bar");
  if(!barWrap) return;
  const ta = barWrap.querySelector("textarea[data-sec][data-idx]");
  if(!ta) return;

  try{ e.preventDefault(); }catch(err){}
  try{ e.stopPropagation(); }catch(err){}

  const beats = Array.from(barWrap.querySelectorAll(".beats .beat"));
  const bi = beats.indexOf(beatEl);
  if(bi < 0) return;

  const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
  const secKey = ta.getAttribute("data-sec");
  const idx = parseInt(ta.getAttribute("data-idx")||"0",10) || 0;
  const sec = p && p.sections ? p.sections[secKey] : null;
  if(!sec || !sec.bars) return;
  ...
  try{ updateDockForKeyboard(); }catch(err){}
}, { passive:false });
