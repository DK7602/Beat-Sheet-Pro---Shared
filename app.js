...
function syncHeaderHeightVar(){
  ...
  const measure = ()=>{
    ...
    if(!collapsed){
      // Measure the full visible header content so the recordings panel never gets squeezed
      // into a sliver on mobile or desktop.
      header.style.height = "";
      header.style.maxHeight = "none";
      header.style.overflow = "visible";
      if(hs){
        hs.style.height = "auto";
        hs.style.overflowY = "visible";
      }

      const fullContentHeight = hs
        ? Math.ceil(hs.scrollHeight || hs.getBoundingClientRect().height || 0)
        : Math.ceil(header.scrollHeight || header.getBoundingClientRect().height || 0);

      h = Math.max(0, fullContentHeight);
...
window.visualViewport?.addEventListener("resize", ()=>syncHeaderHeightVar());
window.visualViewport?.addEventListener("scroll", ()=>syncHeaderHeightVar());
...
function holdCurrentAutoScrollTick(){
  clearPausedHoldVisual();
  try{
    if(lastAutoScrollLitEl && lastAutoScrollLitEl.classList){
      lastAutoScrollLitEl.classList.add("pausedHold");
    }
  }catch(_e){}
}
