...
function renderAutoScrollGlyph(){
  if(!autoScrollOn){
    return `
      <span class="glyphIcon glyphScroll" aria-hidden="true">
        <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
          <path d="M8 4v16"></path>
          <path d="M8 4l-3 3"></path>
          <path d="M8 4l3 3"></path>
          <path d="M16 20V4"></path>
          <path d="M16 20l-3-3"></path>
  ...
  if(autoScrollPaused){
    return `
      <span class="glyphIcon glyphPlay" aria-hidden="true">
        <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
          <path d="M8 6l10 6-10 6z" class="glyphFill"></path>
        </svg>
      </span>`;
  }
  return `
    <span class="glyphIcon glyphPause" aria-hidden="true">
      <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
...
function renderAutoScrollStopGlyph(){
  return `
    <span class="glyphIcon glyphStop" aria-hidden="true">
      <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
...
function updateAutoScrollBtn(){
  ...
  if(els.autoScrollStopBtn){
    ...
    els.autoScrollStopBtn.classList.toggle("show", showStop);
...
function resumeAutoScroll(){
  ...
  if(p){
    p.playback = p.playback || {};
    const visibleKey = getVisibleRealPageKeyFromPager(p) || p.playback.anchorPageKey || p.activeSection || null;
    if(visibleKey){
      p.playback.anchorPageKey = visibleKey;
      if(p.activeSection !== visibleKey) p.activeSection = visibleKey;
    }
  }
  ...
  if(playback && playback.isPlaying){
    ...
    practiceBeat16 = Math.max(0, Math.round(autoScrollPauseBeatPos * 4));
...
function startAutoScrollFromCurrentView(p){
  if(p){
...
function setAutoScroll(v){
  ...
  if(turningOn){
    ...
    autoScrollPauseBeatPos = 0;
    autoScrollPauseRel16 = 0;
    startAutoScrollFromCurrentView(p);
  }
...
function startMetronome(){
  ...
  const tick = () => {
    ...
    autoScrollPauseBeatPos = Math.max(0, (typeof barIdx === "number" ? (barIdx * 4 + beatInBar) : 0));
...
function stopMetronome(){
  ...
  if(autoScrollOn && !autoScrollPaused && !playback.isPlaying && !recording){
    const p = getActiveProject();
    practiceBeat16 = Math.max(0, Math.round(autoScrollPauseBeatPos * 4));
    startPracticeScroll(p);
    return;
...
const playback = {
  ...
  stop(fromEnded){
    ...
    if(autoScrollOn && !autoScrollPaused && !metroOn && !recording){
      const p = getActiveProject();
      practiceBeat16 = Math.max(0, Math.round(autoScrollPauseBeatPos * 4));
      startPracticeScroll(p);
    }else if(!metroOn && !recording){
      clearAllPracticeAndActive();
    }
  },
  ...
  _startSyncLoop(){
    const loop = () => {
      ...
      autoScrollPauseBeatPos = beatPos;
