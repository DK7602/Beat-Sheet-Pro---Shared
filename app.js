...
function attachPullToRefresh(scrollEl, onRefresh){
  ...
  const THRESH = 220; // px - hard pull (less sensitive)
  const MAX_X = 60;   // ignore diagonal/horizontal drags
  const MIN_MS = 260;  // must be a deliberate pull

  const isTypingEl = (el)=>{
    if(!el) return false;
    if(el.closest){
      if(el.closest("textarea,input,select,button,label,.rhymeDock,.dockHideBtn,.chip,.beatBox,.syllBox")) return true;
    }
