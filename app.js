...
let autoScrollHoldConsumed = false;
...
function updateAutoScrollBtn(){
  ...
  els.autoScrollBtn.textContent = !autoScrollOn ? "Scroll" : (autoScrollPaused ? "Resume" : "Pause");
  els.autoScrollBtn.title = !autoScrollOn
    ? "Tap to start auto scroll"
    : (autoScrollPaused ? "Tap to resume • Hold to stop" : "Tap to pause • Hold to stop");
}
...
function syncAutoScrollAnchorFromViewport(p){
  if(!p) return;
...
function getMainBarsScroller(){
  return els.barsScroller || document.getElementById("bars") || els.bars || document.scrollingElement || document.documentElement;
}

function captureAutoScrollPauseMeta(){
  const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
  const scroller = getMainBarsScroller();
  const visibleKey = (p && (getVisibleRealPageKeyFromPager(p) || p.activeSection)) || null;
  const currentPageKey = lastActiveBarKey || visibleKey || (p && p.activeSection) || null;
  const currentBarIdx = Number.isFinite(+lastActiveBarIdx) && lastActiveBarIdx >= 0
    ? +lastActiveBarIdx
    : ((currentPageKey === "full") ? (getFirstVisibleFullPerfTarget()?.barIdx || 0) : getFirstVisibleBarIdxInActivePage());
  const currentSectionKey = (currentPageKey === "full")
    ? (getFirstVisibleFullPerfTarget()?.secKey || null)
    : currentPageKey;

  autoScrollPauseMeta.pageKey = currentPageKey;
  autoScrollPauseMeta.sectionKey = currentSectionKey;
  autoScrollPauseMeta.barIdx = currentBarIdx;
  autoScrollPauseMeta.scrollTop = scroller ? (scroller.scrollTop || 0) : 0;

  if(metroOn){
    const rel16 = Math.max(0, metroBeat16 - (typeof autoScrollMetroOffset16 === "number" ? autoScrollMetroOffset16 : 0));
    autoScrollPauseMeta.rel16InBar = ((rel16 % 16) + 16) % 16;
  }else if(playback && playback.isPlaying){
    const t = (playerEl && typeof playerEl.currentTime === "number") ? playerEl.currentTime : 0;
    const bpmNow = getProjectBpm?.() || 120;
    const beatPos = Math.max(0, ((t * bpmNow) / 60) - (playback.beatOffsetBeats || 0));
    autoScrollPauseMeta.beatInBarFloat = ((beatPos % 4) + 4) % 4;
  }else{
    const rel16 = practiceBeat16 || 0;
    autoScrollPauseMeta.rel16InBar = ((rel16 % 16) + 16) % 16;
  }
}

function restoreAutoScrollPauseMeta(){
  const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
  if(!p) return;

  p.playback = p.playback || {};

  const pausedPageKey = autoScrollPauseMeta.pageKey || p.activeSection || "full";
  const pausedSectionKey = autoScrollPauseMeta.sectionKey || (pausedPageKey === "full" ? null : pausedPageKey);
  const pausedBarIdx = Number.isFinite(+autoScrollPauseMeta.barIdx) ? +autoScrollPauseMeta.barIdx : 0;

  p.playback.anchorPageKey = pausedPageKey;
  if(pausedPageKey !== "full"){
    p.activeSection = pausedPageKey;
  }
  p.playback.seqStartOffset = computeSeqStartOffsetFromAnchor(p, pausedSectionKey || pausedPageKey, pausedBarIdx);

  if(pausedPageKey !== "full" && typeof gotoSectionKey === "function"){
    gotoSectionKey(p, pausedPageKey, "auto");
  }

  const scroller = getMainBarsScroller();
  if(scroller){
    requestAnimationFrame(()=>{
      try{ scroller.scrollTop = autoScrollPauseMeta.scrollTop || 0; }catch(_e){}
    });
  }

  const pausedBeat = Math.max(0, Math.min(3, Math.floor(autoScrollPauseMeta.rel16InBar / 4)));
  if(pausedPageKey === "full"){
    const fullLine = pausedSectionKey ? getFullPerfLineEl(pausedSectionKey, pausedBarIdx) : null;
    if(fullLine){
      clearOldActiveBar();
      fullLine.classList.add("barActive");
      lastActiveBarKey = "full";
      lastActiveBarIdx = pausedBarIdx;
      flashBeatOnFullPerfLine(fullLine, pausedBeat);
    }
  }else{
    const pausedBar = setActiveBarDOM(pausedSectionKey || pausedPageKey, pausedBarIdx);
    if(pausedBar) flashBeatOnBar(pausedBar, pausedBeat);
  }
  lastAutoScrollToken = null;
}

function startAutoScroll(){
  const turningOn = !autoScrollOn;
  autoScrollOn = true;
  ...
  if(turningOn){
    syncAutoScrollAnchorFromViewport(p);
  ...
  if(!metroOn && !(playback && playback.isPlaying)){
    startPracticeScroll(p, autoScrollPauseMeta.practiceBeat16 || 0, !turningOn);
  }
...
function pauseAutoScroll(){
  if(!autoScrollOn || autoScrollPaused) return;
  autoScrollPaused = true;
  captureAutoScrollPauseMeta();
  autoScrollPauseMeta.practiceBeat16 = practiceBeat16 || 0;
  autoScrollPauseMeta.metroBeat16 = metroBeat16 || 0;
  if(playback && playback.isPlaying){
    const t = (playerEl && typeof playerEl.currentTime === "number") ? playerEl.currentTime : 0;
    const bpmNow = getProjectBpm?.() || 120;
    autoScrollPauseMeta.audioBeatPos = Math.max(0, ((t * bpmNow) / 60) - (playback.beatOffsetBeats || 0));
  }else{
    autoScrollPauseMeta.audioBeatPos = 0;
  }
...
function resumeAutoScroll(){
  if(!autoScrollOn || !autoScrollPaused) return;
  restoreAutoScrollPauseMeta();
  autoScrollPaused = false;

  if(metroOn){
    autoScrollMetroOffset16 = metroBeat16 - (autoScrollPauseMeta.rel16InBar || 0);
  }
  if(playback && playback.isPlaying){
    const t = (playerEl && typeof playerEl.currentTime === "number") ? playerEl.currentTime : 0;
    const bpmNow = getProjectBpm?.() || 120;
    playback.beatOffsetBeats = ((t * bpmNow) / 60) - (autoScrollPauseMeta.beatInBarFloat || 0);
  }
  if(!metroOn && !(playback && playback.isPlaying)){
    startPracticeScroll(getActiveProject?.(), autoScrollPauseMeta.rel16InBar || 0, true);
  }
...
function stopAutoScroll(){
  ...
  autoScrollMetroOffset16 = 0;
  autoScrollPauseMeta.practiceBeat16 = 0;
  autoScrollPauseMeta.metroBeat16 = 0;
  autoScrollPauseMeta.audioBeatPos = 0;
  autoScrollPauseMeta.pageKey = null;
  autoScrollPauseMeta.sectionKey = null;
  autoScrollPauseMeta.barIdx = 0;
  autoScrollPauseMeta.rel16InBar = 0;
  autoScrollPauseMeta.beatInBarFloat = 0;
  autoScrollPauseMeta.scrollTop = 0;
...
els.autoScrollBtn?.addEventListener("click", ()=>{
  if(autoScrollHoldConsumed){
    autoScrollHoldConsumed = false;
    return;
  }
...
(function wireAutoScrollHoldToStop(){
...
function startPracticeScroll(p, startBeat16=0, keepAnchor=false){
  stopPracticeScroll(false);
  practiceScrollOn = true;
  practiceBeat16 = Math.max(0, Number(startBeat16) || 0);

  const activeProject = (typeof getActiveProject === "function") ? getActiveProject() : p;
  if(activeProject){
    activeProject.playback = activeProject.playback || {};
    if(!keepAnchor || !activeProject.playback.anchorPageKey){
      syncAutoScrollAnchorFromViewport(activeProject);
    }
  }
...
const playback = {
  ...
  async playRec(rec){
    ...
    if(autoScrollOn){
      this.beatOffsetBeats = 0;
      const ap = (getActiveProject && getActiveProject()) || null;
      playback.anchorPageKey = ap?.activeSection || null;
      if(ap){
        ap.playback = ap.playback || {};
        if(!ap.playback.anchorPageKey) ap.playback.anchorPageKey = playback.anchorPageKey;
      }
    } else {
      this.beatOffsetBeats = 0;
      playback.anchorPageKey = null;
