...
function attachPullToRefresh(scrollEl, onRefresh){
  ...
  const MIN_MS = 140; // must be a deliberate pull
...
const canStartFromEventTarget = (tgt)=>{
  if(!(tgt && scrollEl.contains(tgt))) return false;
  if(outerScrollTop() > 0) return false;
  if((scrollEl.scrollTop || 0) > 0) return false; // MUST be at top of main scroller
...
const move = (clientX, clientY, tgt, prevent)=>{
  ...
  if(!(tgt && scrollEl.contains(tgt))){ reset(); return; }

  if(outerScrollTop() > 0){ reset(); return; }
  if((scrollEl.scrollTop || 0) > 0){ reset(); return; }
