...
function resumeAutoScroll(){
  ...
  if(metroOn){
    autoScrollMetroOffset16 += (metroBeat16 - (autoScrollPauseMeta.metroBeat16 || 0));
  }
  if(playback && playback.isPlaying){
    const t = (playerEl && typeof playerEl.currentTime === "number") ? playerEl.currentTime : 0;
    const bpmNow = getProjectBpm?.() || 120;
    playback.beatOffsetBeats = ((t * bpmNow) / 60) - (autoScrollPauseMeta.audioBeatPos || 0);
  }
  if(!metroOn && !(playback && playback.isPlaying)){
    startPracticeScroll(getActiveProject?.(), autoScrollPauseMeta.practiceBeat16 || 0, true);
  }
