...
function setAutoScroll(v){
  ...
  autoScrollOn = !!v;
  if(!autoScrollOn){ stopPracticeScroll(); }
  ...
  if(turningOn){
    ...
    if(playback && playback.isPlaying){
      ...
      const bpmNow = +((p && p.bpm) || (els.bpm && els.bpm.value) || 120);
    ...
    if(typeof metroBeat16 === "number" && metroTimer){
...
function startPracticeScroll(){
  if(practiceScrollTimer) return;
  practiceBeat16 = 0;

  let nextAt = performance.now();
  const tick = () => {
    // If user started audio/drums, hand off to those clocks
    if(!autoScrollOn || metroOn || (playback && playback.isPlaying)){
      stopPracticeScroll();
      return;
    }
    ...
    const p = getActiveProject?.();
    if(p){
      const beatInBar = (practiceBeat16 % 16);
      const barIdx = Math.floor(practiceBeat16 / 16);
      syncHighlightAndScroll(p, beatInBar, barIdx, performance.now());
    }

    practiceBeat16++;

    const bpmNow = (typeof bpm === "number" && bpm > 0) ? bpm : 90;
    const stepMs = (60_000 / bpmNow) / 4; // 16th notes
    nextAt += stepMs;
    const delay = Math.max(0, nextAt - performance.now());
    practiceScrollTimer = setTimeout(tick, delay);
  };

  practiceScrollTimer = setTimeout(tick, 0);
}
