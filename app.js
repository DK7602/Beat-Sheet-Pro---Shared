...
function getAutoScrollIconHtml(mode){
  ...
  return `<span class="autoIcon autoUpDown" aria-hidden="true"><span>↑</span><span>↓</span></span>`;
}
...
function rememberAutoScrollPausePoint(pageKey, barIdx, beatInBar){
...
function resumeAutoScroll(){
  ...
  const desiredBeatPos = Math.max(0, autoScrollPauseBarIdx * 4 + autoScrollPauseBeatInBar);
  const desired16 = Math.max(0, autoScrollPauseBarIdx * 16 + autoScrollPauseBeatInBar * 4);
...
function startPracticeScroll(p, preservePosition=false){
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  if(autoScrollOn){
    rememberAutoScrollPausePoint(pageKey, globalBarIdx, safeBeat);
  }
  if(autoScrollOn && autoScrollPaused) return;
