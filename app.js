...
function snapshotAutoScrollState(p){
  ...
  try{
    if(playback && playback.isPlaying){
      beatInBar = Math.max(0, Math.min(3, Math.floor(playbackBeats) % 4));
    }else if(metroOn){
      const rel16 = metroBeat16 - (typeof autoScrollMetroOffset16 === "number" ? autoScrollMetroOffset16 : 0);
      beatInBar = Math.max(0, Math.min(3, Math.floor((((rel16 % 16) + 16) % 16) / 4)));
    }else{
      beatInBar = Math.max(0, Math.min(3, Math.floor((((practiceBeat16 % 16) + 16) % 16) / 4)));
    }
  }catch(_e){}
  return {
...
function clearPausedBeatHold(){
  try{
    document.querySelectorAll(".beat.hold, .q.hold").forEach(el=>el.classList.remove("hold"));
  }catch(_e){}
}

function applyPausedBeatHold(snap){
  try{
    clearPausedBeatHold();
    const beatIdx = Math.max(0, Math.min(3, Number.isFinite(+snap?.beatInBar) ? (+snap.beatInBar) : 0));
    const pageKey = snap?.activeBarKey || snap?.anchorPageKey || null;
    const barIdx = Number.isFinite(+snap?.activeBarIdx) ? (+snap.activeBarIdx) : null;
    if(!pageKey || barIdx == null) return;
    ...
    const targets = pageKey === "full" ? bar.querySelectorAll(".q") : bar.querySelectorAll(".beat");
    if(!targets || targets.length < 4) return;
    bar.classList.add("barActive");
    targets[beatIdx]?.classList.add("hold");
  }catch(_e){}
}
...
function setAutoScrollPaused(v){
  ...
  if(next){
    ...
    applyPausedBeatHold(autoScrollPauseSnapshot);
    updateAutoScrollBtn();
...
function clearOldActiveBar(){
  ...
  if(oldPage){
    ...
    oldPage.querySelectorAll(".beat.flash, .q.flash, .beat.hold, .q.hold").forEach(el=>el.classList.remove("flash","hold"));
  }
}
