...
function stopPracticeScroll(){
  if(practiceScrollTimer){
    clearTimeout(practiceScrollTimer);
...
function startPracticeScroll(){
  ...
  // Use the current project BPM; fall back safely.
  const bpmNow = getProjectBpm?.() || Number(els?.bpmInput?.value) || 95;
  const stepMs = 60000 / (bpmNow * 4); // 16th-note interval

  // Anchor to the FIRST VISIBLE BAR in the ACTIVE page
  practiceAnchorPageKey = getActivePageKey?.() || (project?.activePageKey) || null;
  practiceAnchorStartBar = findFirstVisibleBarIndexInPage(practiceAnchorPageKey);

  practiceStartStamp = performance.now();
  practiceBeat16 = 0;

  practiceScrollTimer = setInterval(() => {
    if(!practiceScrollOn || !autoScrollOn) return;

    const now = performance.now();
    const elapsed = now - practiceStartStamp;
    const newBeat16 = Math.floor(elapsed / stepMs);
    if(newBeat16 === practiceBeat16) return;
    practiceBeat16 = newBeat16;

    const relBeat16 = practiceBeat16;
    const beatInBar = Math.floor((relBeat16 % 16) / 4); // 0..3
    const barIdx = practiceAnchorStartBar + Math.floor(relBeat16 / 16);

    // Highlight + scroll using the SAME code path as drums/audio
    const pageKey = practiceAnchorPageKey || getActivePageKey?.() || null;
    syncHighlightAndScroll(pageKey, barIdx, beatInBar, 0);
  }, 30);
}
