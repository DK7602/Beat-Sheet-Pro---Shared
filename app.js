...
function setAutoScroll(v){
  ...
  if(turningOn){
    ...
    if(p){
      ...
      // ✅ If user enables Scroll while viewing FULL, start from the first FULL line in view.
      if(visibleKey === "full"){
...
/***********************
✅ rhymes: beat-box tap delegation (guaranteed)
***********************/
document.addEventListener("click", (e)=>{
...
/***********************
✅ pager helpers (clone-safe) — used by FULL autoscroll + overlay
***********************/
function getPagerEl(){
...
/***********************
✅ FULL VIEW OVERLAY — ensure helper (was missing)
***********************/
...
function ensureFullSectionOverlays(fullPageEl){
  ...
  for(const secEl of sections){
    ...
    let overlay = secEl.querySelector(".fullOverlayWrap");
    if(!overlay){
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  // ✅ If the user is currently viewing FULL, drive the FULL overlay/highlight even if p.activeSection is stale
  const visibleKey = getVisiblePagerKey();
  if(visibleKey === "full") pageKey = "full";

  // ✅ FULL page participates when AutoScroll is ON (flash-highlight the matching beat text)
  if(pageKey === "full"){
...
function renderSectionBarsInto(p, sectionKey, mountEl){
  ...
  sec.bars.forEach((bar, idx)=>{
    ...
    // ✅ Tap a beat box to get rhymes for the PREVIOUS beat's last word
...
/***********************
✅ GLOBAL BEAT TAP -> RHYMES (reliable on mobile)
***********************/
