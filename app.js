...
function ensureFullFlashCss(){
  ...
  css.textContent = `
  /* Full Song View beat-highlight overlay */
...
function ensureFullSectionOverlays(fullPageEl){
  ...
  for(const secEl of sections){
    ...
    const bodyWrap = secEl.querySelector(".fullBodyWrap") || secEl;
    ...
    // Keep overlay height synced with textarea height
    ...
    const html = bars.map((b, bi)=>{
      ...
      return `<span class="fullOverlayLine" data-sec="${escapeHtml(secKey)}" data-bar="${bi}">
        <span class="fullBeat" data-beat="0">${escapeHtml(beats[0]||"")}</span> <span class="fullBeat" data-beat="1">${escapeHtml(beats[1]||"")}</span> <span class="fullBeat" data-beat="2">${escapeHtml(beats[2]||"")}</span> <span class="fullBeat" data-beat="3">${escapeHtml(beats[3]||"")}</span>
      </span>`;
    }).join("\n");
    overlay.querySelector(".fullOverlayText").innerHTML = html;

    // When overlay is ON, hide textarea text so overlay is what you see
    // (textarea remains there for layout + scroll)
    if(secEl.classList.contains("fullOverlayOn")){
...
function setFullOverlayMode(fullPageEl, on){
  try{
    const sections = Array.from(fullPageEl.querySelectorAll(".fullSection"));
    for(const secEl of sections){
      const ta = secEl.querySelector("textarea.fullSectionBody");
