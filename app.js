...
const els = {
  ...
  autoScrollStopBtn: need("autoScrollStopBtn"),
...
let autoScrollPauseBarIdx = 0;
let autoScrollPauseBeatInBar = 0;
let autoScrollPausePageKey = null;
let autoScrollPauseSeqIdx = null;
...
function getAutoScrollIconHtml(mode){
  if(mode === "pause") return `<span class="autoIcon autoPauseBars" aria-hidden="true"><span></span><span></span></span>`;
  if(mode === "play") return `<span class="autoIcon autoPlayTri" aria-hidden="true"></span>`;
  return `<span class="autoIcon autoScrollWord" aria-hidden="true">Scroll</span>`;
}
...
function applyPausedHighlight(){
  clearPausedHighlight();
  try{
    const beatIdx = Math.max(0, Math.min(3, autoScrollPauseBeatInBar|0));
    if(lastActiveBarKey === 'full'){
      const lineEl = document.querySelector('.page[data-page-key="full"]:not([data-clone="1"]) .fullPerfLine.barActive');
      const target = lineEl?.querySelectorAll('.q')?.[beatIdx];
      if(target) target.classList.add('pausedHold');
      return;
    }
    const barEl = document.querySelector(`.page[data-page-key="${CSS.escape(String(lastActiveBarKey || ''))}"]:not([data-clone="1"]) .bar.barActive`);
    const target = barEl?.querySelectorAll('.beat')?.[beatIdx];
    if(target) target.classList.add('pausedHold');
  }catch(_e){}
}
function rememberAutoScrollPausePoint(pageKey, barIdx, beatInBar, seqIdx=null){
  autoScrollPausePageKey = pageKey || autoScrollPausePageKey || null;
  autoScrollPauseBarIdx = Math.max(0, Number.isFinite(+barIdx) ? (+barIdx|0) : 0);
  autoScrollPauseBeatInBar = Math.max(0, Math.min(3, Number.isFinite(+beatInBar) ? (+beatInBar|0) : 0));
  autoScrollPauseSeqIdx = Number.isFinite(+seqIdx) ? (+seqIdx|0) : null;
}
function pauseAutoScroll(){
  if(!autoScrollOn || autoScrollPaused) return;
  autoScrollPaused = true;
  stopPracticeScroll();
  applyPausedHighlight();
  updateAutoScrollBtn();
...
function resumeAutoScroll(){
  ...
  if(p){
    ...
    if(autoScrollPausePageKey){
      p.playback.anchorPageKey = autoScrollPausePageKey;
      if(p.activeSection !== autoScrollPausePageKey) p.activeSection = autoScrollPausePageKey;
    }
  }

  const baseSeqOffset = Math.max(0, Number.isFinite(+(p?.playback?.seqStartOffset)) ? (+(p.playback.seqStartOffset)|0) : 0);
  const desiredGlobalBarIdx = (autoScrollPauseSeqIdx != null)
    ? Math.max(0, autoScrollPauseSeqIdx - baseSeqOffset)
    : Math.max(0, autoScrollPauseBarIdx);
  const desiredBeatPos = Math.max(0, desiredGlobalBarIdx * 4 + autoScrollPauseBeatInBar);
  const desired16 = Math.max(0, desiredGlobalBarIdx * 16 + autoScrollPauseBeatInBar * 4);
  ...
  if(playback && playback.isPlaying){
    ...
    playback.beatOffsetBeats = (t * bpmNow) / 60 - desiredBeatPos;
  }

  if(typeof metroBeat16 === 'number' && metroOn){
    autoScrollMetroOffset16 = metroBeat16 - desired16;
  }

  if(!metroOn && !(playback && playback.isPlaying)) {
    practiceBeat16 = desired16;
    startPracticeScroll(p);
  }

  updateAutoScrollBtn();
  showToast('Auto Scroll Resumed');
}
function stopAutoScrollCompletely(){
  autoScrollPaused = false;
  clearPausedHighlight();
  setAutoScroll(false);
}
function updateAutoScrollBtn(){
  ...
  const isRunning = !!autoScrollOn && !autoScrollPaused;
  try{
  ...
  els.autoScrollBtn.innerHTML = getAutoScrollIconHtml(autoScrollOn ? (autoScrollPaused ? 'play' : 'pause') : 'idle');
  els.autoScrollBtn.title = !autoScrollOn ? 'Auto Scroll' : (autoScrollPaused ? 'Resume Auto Scroll' : 'Pause Auto Scroll');
  els.autoScrollBtn.setAttribute('aria-label', els.autoScrollBtn.title);
  if(els.autoScrollStopBtn){
    els.autoScrollStopBtn.hidden = !autoScrollOn;
    els.autoScrollStopBtn.style.display = autoScrollOn ? '' : 'none';
...
function setAutoScroll(v){
  ...
  if(!autoScrollOn){
    ...
    autoScrollPauseBarIdx = 0;
    autoScrollPauseBeatInBar = 0;
    autoScrollPausePageKey = null;
    autoScrollPauseSeqIdx = null;
  ...
  if(turningOn){
    if(p){
      ...
      autoScrollPausePageKey = visibleKey || null;
    }
...
els.autoScrollStopBtn?.addEventListener("click", ()=> stopAutoScrollCompletely());
/***********************
...
function startPracticeScroll(p){
  const resumeBeat16 = Math.max(0, practiceBeat16|0);
  stopPracticeScroll();
  practiceScrollOn = true;
  practiceBeat16 = resumeBeat16;
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  if(pageKey === "full"){
    ...
    rememberAutoScrollPausePoint("full", target.barIdx, safeBeat, idx);
  ...
  if(!seq.length){
    ...
    rememberAutoScrollPausePoint(pageKey, safeBarIdx, safeBeat, safeBarIdx);
  ...
  rememberAutoScrollPausePoint(target.secKey, target.barIdx, safeBeat, idx);
