...
let autoScrollPauseBeatPos = 0;
let autoScrollPauseBeatIndex = 0;
let currentAutoScrollVisual = { pageKey:null, secKey:null, barIdx:-1, beatInBar:0 };
...
function renderAutoScrollGlyph(){
  if(!autoScrollOn) return '<span class="scrollGlyph" aria-hidden="true">↑↓</span>';
  if(autoScrollPaused) return '<span class="playGlyph" aria-hidden="true"></span>';
  return '<span class="pauseGlyph" aria-hidden="true"><span></span><span></span></span>';
}
function renderAutoScrollStopGlyph(){
  return '<span class="stopGlyph" aria-hidden="true"></span>';
}
function updateAutoScrollBtn(){
  ...
  els.autoScrollBtn.innerHTML = renderAutoScrollGlyph();
  els.autoScrollBtn.title = !autoScrollOn
    ? "Auto Scroll"
    : (autoScrollPaused ? "Resume Auto Scroll" : "Pause Auto Scroll");
  els.autoScrollBtn.setAttribute("aria-label", !autoScrollOn
    ? "Auto Scroll"
    : (autoScrollPaused ? "Resume Auto Scroll" : "Pause Auto Scroll"));

  if(els.autoScrollStopBtn){
    const showStop = !!autoScrollOn;
    els.autoScrollStopBtn.hidden = !showStop;
    els.autoScrollStopBtn.style.display = showStop ? "flex" : "none";
    els.autoScrollStopBtn.innerHTML = renderAutoScrollStopGlyph();
    els.autoScrollStopBtn.title = "Stop Auto Scroll";
    els.autoScrollStopBtn.setAttribute("aria-label", "Stop Auto Scroll");
  }
}
...
function showPausedHighlight(){
  clearPausedHighlight();
  const beatIdx = Math.max(0, Math.min(3, autoScrollPauseBeatIndex|0));
  const v = currentAutoScrollVisual || {};
  if(v.pageKey === 'full'){
    const lineEl = getFullPerfLineEl(v.secKey, v.barIdx);
    if(!lineEl) return;
    const qs = lineEl.querySelectorAll('.q');
    if(qs && qs[beatIdx]) qs[beatIdx].classList.add('pausedHold');
    return;
  }
  const pageEl = getActiveRealPageEl(v.pageKey || lastActiveBarKey);
  if(!pageEl) return;
  const barEl = pageEl.querySelector(`.bar[data-bar-idx="${v.barIdx >= 0 ? v.barIdx : lastActiveBarIdx}"]`);
  if(!barEl) return;
  const beats = barEl.querySelectorAll('.beat');
  if(beats && beats[beatIdx]) beats[beatIdx].classList.add('pausedHold');
}
function captureAutoScrollPausePoint(p){
  ...
  if(playback && playback.isPlaying){
    ...
    autoScrollPauseBeatPos = Math.max(0, (t * bpmNow) / 60 - (playback.beatOffsetBeats || 0));
  }else if(metroOn){
    const rel16 = metroBeat16 - (typeof autoScrollMetroOffset16 === 'number' ? autoScrollMetroOffset16 : 0);
    autoScrollPauseBeatPos = Math.max(0, rel16 / 4);
  }else if(practiceScrollOn || practiceBeat16 > 0){
    autoScrollPauseBeatPos = Math.max(0, practiceBeat16 / 4);
  }else{
    autoScrollPauseBeatPos = 0;
  }
  autoScrollPauseBeatIndex = Math.max(0, Math.min(3, Math.floor(autoScrollPauseBeatPos) % 4));
}
function pauseAutoScroll(){
  if(!autoScrollOn || autoScrollPaused) return;
  const p = (typeof getActiveProject === "function") ? getActiveProject() : null;
  captureAutoScrollPausePoint(p);
...
function resumeAutoScroll(){
  ...
  if(playback && playback.isPlaying){
    const t = (playerEl && typeof playerEl.currentTime === "number") ? playerEl.currentTime : 0;
    playback.beatOffsetBeats = (t * bpmNow) / 60 - autoScrollPauseBeatPos;
  }else if(metroOn){
    autoScrollMetroOffset16 = metroBeat16 - Math.round(autoScrollPauseBeatPos * 4);
  }else{
    startPracticeScroll(p, Math.max(0, Math.round(autoScrollPauseBeatPos * 4)), true);
  }
...
function handleAutoScrollMainButton(){
  if(!autoScrollOn){
    setAutoScroll(true);
    return;
  }
...
function setAutoScroll(v){
  ...
  autoScrollOn = !!v;
  if(!autoScrollOn) autoScrollPaused = false;
...
els.autoScrollBtn?.addEventListener("click", handleAutoScrollMainButton);
els.autoScrollStopBtn?.addEventListener("click", ()=> setAutoScroll(false));
/***********************
...
function startPracticeScroll(p, startBeat16=0, preserveAnchor=false){
  stopPracticeScroll();
  practiceScrollOn = true;
  practiceBeat16 = Math.max(0, startBeat16|0);

  const activeProject = (typeof getActiveProject === "function") ? getActiveProject() : p;
  if(activeProject){
    activeProject.playback = activeProject.playback || {};
    const visibleKey = preserveAnchor ? (activeProject.playback.anchorPageKey || activeProject.activeSection || null) : (getVisibleRealPageKeyFromPager(activeProject) || activeProject.activeSection || null);
    if(visibleKey){
    ...
    const firstVisibleBarIdx = preserveAnchor ? 0 : getFirstVisibleBarIdxInActivePage();
...
function flashBeatOnBar(barEl, beatInBar){
  ...
  beats.forEach(b=>b.classList.remove("flash", "pausedHold"));
  const t = beats[beatInBar];
  if(t) t.classList.add("flash");
  setTimeout(()=>{ if(!autoScrollPaused) beats.forEach(b=>b.classList.remove("flash")); }, 90);
}
...
function flashBeatOnFullPerfLine(lineEl, beatInBar){
  ...
  qs.forEach(q=>q.classList.remove('flash', 'pausedHold'));
  const t = qs[beatInBar];
  if(t) t.classList.add('flash');
  setTimeout(()=>{ if(!autoScrollPaused) qs.forEach(q=>q.classList.remove('flash')); }, 90);
}
...
function clearAllPracticeAndActive(){
  document.querySelectorAll(".bar.barActive, .fullPerfLine.barActive").forEach(b=>b.classList.remove("barActive"));
  document.querySelectorAll(".beat.flash, .beat.pausedHold").forEach(b=>b.classList.remove("flash", "pausedHold"));
  document.querySelectorAll(".q.flash, .q.pausedHold").forEach(b=>b.classList.remove("flash", "pausedHold"));
  currentAutoScrollVisual = { pageKey:null, secKey:null, barIdx:-1, beatInBar:0 };
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  if(pageKey === "full"){
    ...
    currentAutoScrollVisual = { pageKey:"full", secKey:target.secKey, barIdx:target.barIdx, beatInBar:safeBeat };
  ...
  if(!seq.length){
    ...
    currentAutoScrollVisual = { pageKey, secKey:pageKey, barIdx:safeBarIdx, beatInBar:safeBeat };
  ...
  currentAutoScrollVisual = { pageKey:target.secKey, secKey:target.secKey, barIdx:target.barIdx, beatInBar:safeBeat };
  if(barEl) flashBeatOnBar(barEl, safeBeat);
