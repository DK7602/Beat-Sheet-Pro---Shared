...
/***********************
✅ Pull-to-refresh (swipe down on Full/Pages list)
***********************/
function setupPullToRefresh(){
  ...
  function onMove(e){
    ...
    // prevent the browser from rubber-banding the whole page
...
function renderFullSongFlow(p, mount){
  ...
  for(const key of keys){
    const sec = p.sections[key] || (p.sections[key] = { key, title:"", bars:[{text:""}], titleEditable:isExtraKey(key) });
    ...
    addBtn.addEventListener("click", (e)=>{
      ...
      const sec2 = p.sections[key] || (p.sections[key] = { key, title:"", bars:[{text:""}], titleEditable:isExtraKey(key) });
...
document.addEventListener("click", (e)=>{
  ...
  if(!btn) return;
  // prevent focus/scroll jumps on mobile
  e.preventDefault();
  e.stopPropagation();
