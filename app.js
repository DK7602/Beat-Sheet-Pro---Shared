...
function getActivePageOrder(p){
  // swipe/card order follows the Full Song View order exactly.
  // pageKeysActive is the source of truth for user-created/reordered sections.
  const seen = new Set();
  const active = [];
  for(const k of (p.pageKeysActive || [])){
    if(!k || k === "full" || seen.has(k)) continue;
    if(!p.sections?.[k]) continue;
    seen.add(k);
    active.push(k);
  }
  return ["full", ...active];
...
function insertPageKeyAt(p, key, index){
  if(!Array.isArray(p.pageKeysActive)) p.pageKeysActive = [];
  p.pageKeysActive = p.pageKeysActive.filter(k => k && k !== "full" && k !== key);
  const safeIndex = Math.max(0, Math.min(Number.isFinite(index) ? index : p.pageKeysActive.length, p.pageKeysActive.length));
  p.pageKeysActive.splice(safeIndex, 0, key);
}
...
function addNextPage(p, fromKey, opts={}){
  ...
  markDeleted(p, key, false);

  // Header + on Full Song View inserts a new section at the beginning.
  // Section + buttons insert immediately after that section.
  if(fromKey === "full"){
    insertPageKeyAt(p, key, 0);
  }else{
    const currentOrder = (p.pageKeysActive || []).filter(k => k && k !== "full");
    const fromIdx = currentOrder.indexOf(fromKey);
    insertPageKeyAt(p, key, fromIdx >= 0 ? fromIdx + 1 : currentOrder.length);
  }
  ...
  touchProject(p);
  // Render and snap exactly once to the intended page (prevents "bounce"/back-scroll).
  renderBars({
...
document.addEventListener("click", (e)=>{
  ...
  const action = btn.getAttribute("data-action");
    ...
    addNextPage(p, key, key === "full" ? { preserveScroll:true, stayOnFull:true } : {});
    return;
  }
