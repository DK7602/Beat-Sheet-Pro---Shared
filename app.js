...
function ensureFullFlashCss(){
  if(_fullFlashCssInjected) return;
  _fullFlashCssInjected = true;
  try{
    const st = document.createElement("style");
    st.id = "fullFlashCss";
    st.textContent = `
      .fullOverlay{ z-index: 3; }
      .fullOverlayLine{ margin: 0 0 10px 0; }
      .fullBeat{ padding: 0 2px; border-radius: 6px; }
      .fullBeat.flash{
        background: #ffe24a;
        box-shadow: 0 0 0 3px rgba(255,226,74,0.95);
      }
    `;
    document.head.appendChild(st);
  }catch(e){}
}
// ---------- FULL SONG VIEW AutoScroll overlay helpers ----------
let fullOverlayBuiltForProj = null;

function getVisiblePageElFromPagerByKey(key){
  try{
    const pagerEl = document.getElementById("pagesPager");
    if(!pagerEl) return null;

    // 1) Try the currently snapped index first (clone-safe)
    const idx = getCurrentIdx(pagerEl);
    const cur = (pagerEl.children && pagerEl.children[idx]) ? pagerEl.children[idx] : null;
    if(cur && cur.dataset && cur.dataset.pageKey === key) return cur;

    // 2) Otherwise, choose the instance of that pageKey that is actually in view
    const cands = Array.from(pagerEl.querySelectorAll(`.page[data-page-key="${key}"]`));
...
function computeSeqStartOffsetFromFullViewport(p){
  try{
    ...
    const scroller = els?.bars || document.scrollingElement;
    const scRect = scroller ? scroller.getBoundingClientRect() : { top:0 };
    ...
    for(const el of lines){
      const r = el.getBoundingClientRect();
      if(r.bottom > scRect.top + 8){ first = el; break; }
    }

    const secKey = first ? (first.getAttribute('data-sec') || '') : '';
    const barIdx = first ? parseInt(first.getAttribute('data-bar') || '0', 10) : 0;
    if(!secKey) return 0;

    const off = seq.findIndex(x => x.secKey === secKey && Number(x.barIdx) === Number(barIdx));
    return off >= 0 ? off : 0;
  }catch(e){
    return 0;
  }
...
function ensureFullOverlays(p, fullPageEl){
  try{
    ensureFullFlashCss();
    const projMark = String(p?.id || '') + '::' + String(p?.updatedAt || '');
    // Rebuild overlays if project changed or overlays missing
    const needs = (!fullOverlayBuiltForProj || fullOverlayBuiltForProj !== projMark || !fullPageEl.querySelector('.fullOverlay'));
    if(!needs) return;

    const mount = fullPageEl.querySelector('.fullSongMount');
    if(!mount) return;

    // Remove old overlays
    mount.querySelectorAll('.fullOverlay').forEach(el=>el.remove());

    // Build per-section overlays
    mount.querySelectorAll('.fullSection').forEach(secEl=>{
      const secKey = secEl.dataset.secKey;
      const ta = secEl.querySelector('textarea.fullSectionEditor');
      if(!secKey || !ta) return;

      secEl.style.position = secEl.style.position || 'relative';

      const ov = document.createElement('div');
      ov.className = 'fullOverlay';
      ov.setAttribute('aria-hidden','true');

      // Position overlay over the textarea
      const left = ta.offsetLeft;
      const top = ta.offsetTop;
      ...
      ov.style.font = 'inherit';
      ov.style.lineHeight = getComputedStyle(ta).lineHeight;
      ov.style.padding = getComputedStyle(ta).padding;
      ov.style.borderRadius = getComputedStyle(ta).borderRadius;

      // Mirror typography/background so it looks identical
      const cs = getComputedStyle(ta);
      ov.style.background = cs.backgroundColor;
      ov.style.color = cs.color;
      ov.style.fontFamily = cs.fontFamily;
      ov.style.fontSize = cs.fontSize;
      ov.style.zIndex = "3";

      // Build overlay HTML from section bars + computeBeats
      const bars = (p.sections?.[secKey]?.bars || []).map(b=>b?.text ?? '');
      const lines = [];
      for(let bi=0; bi<bars.length; bi++){
        const txt = String(bars[bi]||'').replace(/\r/g,'');
        if(!txt.trim()) continue;
        const beats = computeBeats(txt);
        const safe = [beats[0]||'', beats[1]||'', beats[2]||'', beats[3]||''];
        lines.push(
          `<div class="fullOverlayLine" data-sec="${escapeHtml(secKey)}" data-bar="${bi}">`+
            safe.map((t, i)=>`<span class="fullBeat" data-sec="${escapeHtml(secKey)}" data-bar="${bi}" data-beat="${i}">${escapeHtml(t)}</span>`).join(' ')+
          `</div>`
        );
      }
      ov.innerHTML = lines.join('');
      ...
      secEl.appendChild(ov);
    });

    fullOverlayBuiltForProj = projMark;
  }catch(e){
    // never crash
  }
}
...
function setFullOverlayMode(fullPageEl, on){
  try{
    fullPageEl.querySelectorAll('textarea.fullSectionEditor').forEach(ta=>{
      if(on){
        ta.dataset._prevColor = ta.style.color || '';
        ta.dataset._prevCaret = ta.style.caretColor || '';
        ta.style.color = 'transparent';
        ta.style.caretColor = 'transparent';
      }else{
        ta.style.color = ta.dataset._prevColor || '';
        ta.style.caretColor = ta.dataset._prevCaret || '';
      }
    });
    fullPageEl.querySelectorAll('.fullOverlay').forEach(ov=>{
...
function flashFullBeat(fullPageEl, secKey, barIdx, beatIdx){
  try{
    ...
    const esc = (window.CSS && CSS.escape) ? CSS.escape : (s)=>String(s).replace(/[^a-zA-Z0-9_\-]/g,'\$&');
    const sel = `.fullBeat[data-sec="${esc(secKey)}"][data-bar="${barIdx}"][data-beat="${beatIdx}"]`;
    const el = fullPageEl.querySelector(sel);
    if(el){
...
function scrollFullLineIntoView(fullPageEl, secKey, barIdx){
  try{
    ...
    // If line is outside comfortable window, scroll it into the center-ish
    const topThresh = scRect.top + 120;
    const botThresh = scRect.bottom - 180;
    if(r.top < topThresh || r.bottom > botThresh){
