...
function repairProject(p){
  ...
  for(const def of BASE_SECTION_DEFS){
    if(!p.sections[def.key] || typeof p.sections[def.key] !== "object"){
      p.sections[def.key] = { key:def.key, title:def.title, bars:[{text:""}], titleEditable:true };
    }
    ...
    if(!String(p.sections[def.key].title || "").trim()) p.sections[def.key].title = def.title;
...
function buildPager(p){
  ...
  CAROUSEL_ORDER.forEach((key, i)=>{
    ...
    if(key === "full"){
      ...
      titleTxt.value = (sec.title || getHeadingTextForKey(p,key) || "").trim();
