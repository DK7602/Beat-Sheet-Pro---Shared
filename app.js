...
async function initCloud(){
  ...
  try{
    ...
    if(p){
      ...
      renderAll();
      requestAnimationFrame(()=>{ try{ snapPagerToPageKey("full", "auto"); }catch(_e){} });
    }
  }catch(_e){}
}
...
const wantsEchoProtection = !!(metroOn || playback?.isPlaying);
try{
  micStream = await navigator.mediaDevices.getUserMedia({
    audio: {
      // Keep the mic natural, but enable echo protection while drums/backing are playing
      // so the direct drum bus does not get doubled back through speaker bleed.
      echoCancellation: wantsEchoProtection,
      ...
      googEchoCancellation: wantsEchoProtection,
