...
const RHYME_CACHE_KEY = `${KEY_PREFIX}rhyme_cache_v2`;
...
function scoreRhymeCandidate(seed, item, kind){
  ...
  let score = 0;
  if(kind === 'perfect') score += 120;
  if(kind === 'near') score += 82;
  if(kind === 'soundalike') score += 72;

  const sSyl = syllableEstimate(seed);
  const wSyl = syllableEstimate(word);
  score += Math.max(0, 22 - (Math.abs(sSyl - wSyl) * 6));

  if(RHYME_LYRIC_BOOST_WORDS.has(word)) score += 55;
  if(RHYME_THEME_BOOST_RE.test(word)) score += 18;
  if(RHYME_CONVERSATIONAL_BOOST_RE.test(word)) score += 14;

  if(word.length >= 3 && word.length <= 8) score += 16;
  if(word.length <= 2) score -= 6;
  if(word.length > 10) score -= 18;
  if(word.length > 12) score -= 12;
  if(/^(un|re|mis|dis|trans|inter)/.test(word) && word.length > 8) score -= 10;
  if(/[^a-z']/.test(word)) score -= 12;
  if(RHYME_OBSCURE_PENALTY_RE.test(word)) score -= 55;
  if(RHYME_AWKWARD_PENALTY_RE.test(word) && !RHYME_LYRIC_BOOST_WORDS.has(word)) score -= 18;
  if(/[qxzj]{2,}/i.test(word)) score -= 10;
  if(/(?:cked|pted|ctions|ments)$/i.test(word)) score -= 8;

  const numSyllables = Number(item?.numSyllables || 0);
  if(numSyllables){
    score += Math.max(0, 18 - (Math.abs(numSyllables - sSyl) * 5));
  }

  const apiScore = Number(item?.score || 0);
  if(apiScore){
    score += Math.max(0, Math.min(16, Math.log10(Math.max(1, apiScore + 1)) * 7));
  }
  const freq = parseDatamuseFreq(item);
  if(freq){
    score += Math.max(-6, Math.min(24, freq * 6));
  }

  if(rhymeTail(word) === rhymeTail(seed)) score += 12;
  if(vowelTail(word) === vowelTail(seed)) score += 10;
...
function buildRhymeBuckets(seed, perfectData, nearData, soundLikeData){
  ...
  return {
    perfect: perfect.slice(0, 10).map(x=>x.word),
    near: near.slice(0, 10).map(x=>x.word)
  };
}
...
async function updateRhymes(seed){
  ...
  if(Array.isArray(cached) && cached.length){
    const fallbackBuckets = { perfect: cached.slice(0,10), near: [] };
  ...
  try{
    ...
    const [perfectRes, nearRes, soundLikeRes] = await Promise.all([
      fetch(`https://api.datamuse.com/words?rel_rhy=${encodeURIComponent(w)}&md=f&max=40`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&md=f&max=40`, { signal: rhymeAbort.signal }),
      fetch(`https://api.datamuse.com/words?sl=${encodeURIComponent(w)}&md=f&max=30`, { signal: rhymeAbort.signal })
    ]);
...
function renderRhymes(groups){
  ...
  const perfect = Array.isArray(groups) ? groups.slice(0,18) : (Array.isArray(groups?.perfect) ? groups.perfect : []);
  const near = Array.isArray(groups?.near) ? groups.near : [];
  if(!perfect.length && !near.length){
    els.rhymeList.innerHTML = `<span class="small">No rhymes found.</span>`;
    return;
  }
  const renderGroup = (label, words) => {
    if(!words || !words.length) return "";
    return `
      <div class="rhymeGroup">
        <div class="small" style="font-weight:800;margin:6px 0 8px;">${escapeHtml(label)}</div>
        <div class="rhymeGroupChips">${words.map(w=>`<button type="button" class="rhymeChip" data-rhyme="${escapeHtml(w)}">${escapeHtml(w)}</button>`).join("")}</div>
      </div>
    `;
  };
  els.rhymeList.innerHTML = renderGroup("Perfect Rhymes", perfect) + renderGroup("Near Rhymes", near);
}
