...
function getNotesTextBeforeCaret(editor){
  try{
    const sel = window.getSelection?.();
...
function updateRhymesFromNotesCaret(){
  ...
  const before = getNotesTextBeforeCaret(ed);
  const cut = before.lastIndexOf("\n");
  const beforePrevLine = (cut >= 0) ? before.slice(0, cut) : "";
  const lines = beforePrevLine.replace(/\r/g, "").split("\n");
  let base = "";
...
function wireNotesEditor(){
  ...
  ed.addEventListener("keyup", ()=> scheduleNotesRhymeRefresh());
  ed.addEventListener("click", ()=> scheduleNotesRhymeRefresh());
  ed.addEventListener("mouseup", ()=> scheduleNotesRhymeRefresh());
  ed.addEventListener("focus", ()=> scheduleNotesRhymeRefresh());
...
document.addEventListener("selectionchange", ()=>{
  if(notesOpen && document.activeElement === els.notesEditor) scheduleNotesRhymeRefresh();
}, true);
