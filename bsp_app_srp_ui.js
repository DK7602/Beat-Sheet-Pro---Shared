...
function updateRhymesFromSectionCaret(p, key, ta){
  try{
    ...
    // find previous non-empty line within this section
    ...
    // if none in this section, look back through previous sections
    if(!baseLine){
    ...
    if(!baseLine){
      setRhymeBase("Tap into a beat…");
      renderRhymes([]);
      return;
    }

    const m = baseLine.match(/([A-Za-z']+)(?:[^A-Za-z']*)$/);
    const lastWord = (m ? m[1] : "").toLowerCase();
    if(!lastWord){
      setRhymeBase(baseLine.slice(0, 28));
      renderRhymes([]);
      return;
    }

    setRhymeBase(lastWord);
    renderRhymes(getRhymes(lastWord));
  }catch(err){
...
function renderFullSongFlow(p, mount){
  ...
  for(const key of keys){
    const sec = p.sections[key] || (p.sections[key] = { key, title:getHeadingTextForKey(p,key), bars:[{text:""}], titleEditable:isExtraKey(key) });
    ...
    pill.value = (sec.title || getHeadingTextForKey(p,key) || "").trim();
    ...
    pill.addEventListener("input", ()=>{
      ...
      touchProject(p);
      // update page title rows + export labels without forcing a full rebuild
      render(); // light render; safe because it's debounced by browser
    }, { passive:true });

    const ta = document.createElement("textarea");
    ta.className = "fullSectionEditor";
    ta.spellcheck = false;
    ta.value = sectionTextFromProject(p, key);
    ta.dataset.secEditor = key;

    // auto-size immediately
    requestAnimationFrame(()=> autoGrowTextarea(ta));

    const btnRow = document.createElement("div");
    btnRow.className = "fullSectionBtns";
    btnRow.innerHTML = `
      <button type="button" class="barPlusBtn" data-action="addPage" data-page="${escAttr(key)}" aria-label="Add page">+</button>
      <button type="button" class="barDelBtn" data-action="delPage" data-page="${escAttr(key)}" aria-label="Delete page">×</button>
    `;
...
function buildPager(p){
  ...
  box.innerHTML = `
    <div class="fullSub">Full Song View</div>
    <div class="fullSongMount"></div>
  `;
  page.appendChild(box);

  // render SRP-style flow into FULL
  const mount = box.querySelector(".fullSongMount");
