...
function relocateMiniCard(){
  const header = document.querySelector("header");
  const miniCard = document.getElementById("miniCard");
  const belowHeader = document.getElementById("belowHeader");
  if(!miniCard || !belowHeader || !header) return;

  const isCollapsed = document.body.classList.contains("headerCollapsed");
  const isFullMode = document.body.classList.contains("fullMode");

  // In Full Song View with header shown, keep Recordings (miniCard) below the fixed header
  // so the user can scroll down to recordings / uploads.
  if(isFullMode && !isCollapsed){
    if(miniCard.parentElement !== belowHeader){
      belowHeader.prepend(miniCard);
    }
    return;
  }

  // When header is collapsed, keep the miniCard inside the header (hidden) so layout stays compact.
  if(isCollapsed){
    if(miniCard.parentElement !== header){
