...
function syncDockHeightVar(){
  try{
    ...
    const h = (hidden) ? 0 : Math.ceil(dock.getBoundingClientRect().height || 0);
...
function getFirstVisibleBarIdxInActivePage(){
  ...
  try{
    ...
    // ✅ account for sticky header inside the page so we don't "skip" the first bar
...
function getFirstVisibleFullPerfTarget(){
  try{
    ...
    // ✅ account for sticky header inside the full page so we don't "skip" the first line
