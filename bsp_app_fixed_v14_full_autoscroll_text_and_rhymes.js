...
function getVisiblePageElFromPagerByKey(key){
  try{
    ...
    const idx = getCurrentIdx(pagerEl);
    const pageEl = pagerEl.children && pagerEl.children[idx];
    if(pageEl && pageEl.dataset && pageEl.dataset.pageKey === key) return pageEl;
    return pagerEl.querySelector(`.page[data-page-key="${key}"]`);
  }catch(e){
    return null;
  }
}
...
function ensureFullOverlays(p, fullPageEl){
  try{
    ...
    mount.querySelectorAll('.fullSection').forEach(secEl=>{
      ...
      // Mirror background so it looks identical
      ov.style.background = getComputedStyle(ta).backgroundColor;
...
function syncSectionCardsFromProject(p){
  ...
  areas.forEach(ta=>{
    ...
    // ✅ Tapping a beat box updates rhyme suggestions (previous beat's last word)
