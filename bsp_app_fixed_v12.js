...
function relocateMiniCard(){
  const mini = document.querySelector(".miniCard");
  const host = document.getElementById("belowHeader");
  const header = document.querySelector("header");
  if(!mini || !host || !header) return;
  const collapsed = document.body.classList.contains("headerCollapsed");
  if(collapsed){
    if(!header.contains(mini)) header.appendChild(mini);
  } else {
    if(!host.contains(mini)) host.appendChild(mini);
  }
}
...
function startMetronome(){
  ...
  const tick = () => {
    ...
    const p = getActiveProject();
    const pageKey = this.anchorPageKey || p?.activeSection || "full";
...
const playback = {
  ...
  async playRec(rec){
    ...
    if(autoScrollOn){
      this.beatOffsetBeats = 0;
      this.anchorPageKey = (getActiveProject && getActiveProject())?.activeSection || null;
    } else {
      this.beatOffsetBeats = 0;
      this.anchorPageKey = null;
    }
