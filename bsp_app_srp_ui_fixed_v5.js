...
function renderFullSongFlow(p, mount){
  ...
  for(const key of keys){
    ...
    pill.addEventListener("input", ()=>{
      ...
      touchProject(p);
      // update title in this page header (FULL and cards) without rebuilding everything
      const pageEl = mount.closest?.(".page");
      if(pageEl){
        const keyNow = pageEl.dataset.pageKey;
        if(keyNow === "full"){
          // update all matching title rows
          document.querySelectorAll('.page[data-page-key="full"] .pageTitle').forEach(el=>{ el.textContent = "Full Song View"; });
        }
      }
    });
    ...
    addBtn.addEventListener("click", (e)=>{
      ...
      const sec2 = p.sections[key] || (p.sections[key] = { key, title:getHeadingTextForKey(p,key), bars:[{text:""}], titleEditable:isExtraKey(key) });
      sec2.bars = (sec2.bars && sec2.bars.length) ? sec2.bars : [{text:""}];
      sec2.bars.push({ text:"" });
      syncSectionCardsFromProject(p);
      touchProject(p);
      renderSheet();
    ...
    delBtn.addEventListener("click", (e)=>{
      e.preventDefault(); e.stopPropagation();
      const sec2 = p.sections[key];
      if(!sec2) return;
      if(sec2.bars && sec2.bars.length>1) sec2.bars.pop();
      else if(sec2.bars && sec2.bars.length===1) sec2.bars[0].text="";
      syncSectionCardsFromProject(p);
      touchProject(p);
      renderSheet();
    });
