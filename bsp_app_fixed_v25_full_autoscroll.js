...
/***********************
✅ FULL VIEW MODE (edit vs play)
- When AutoScroll is ON and user is on FULL, we render Full as beat boxes so highlight can flash.
***********************/
function ensureFullViewMode(p, wantPlay){
  try{
    ...
    const curPlay = (mount.dataset && mount.dataset.fullMode === "play");
    ...
    renderFullSongFlow(p, mount, { playMode: nextPlay });
...
function setActiveBarDOM(pageKey, barIdx, subSecKey=null){
  ...
  const page = getPageElForKey(getActiveProject(), baseKey);
  if(!page) return null;

  let bar = null;

  if(baseKey === "full"){
    // Full-view play mode: bars are inside .fullSection blocks
    const sk = String(subSecKey || "").trim();
    if(!sk) return null;
    const secEl = page.querySelector(`.fullSection[data-sec-key="${escSel(sk)}"]`);
    if(!secEl) return null;
    bar = secEl.querySelector(`.bar[data-bar-idx="${barIdx}"]`);
  }else{
...
function renderFullSongFlow(p, mount, opts={}){
  ...
  const playMode = !!(opts && opts.playMode);

  // mark mode so other helpers can detect it
  mount.dataset.fullMode = playMode ? "play" : "edit";
  ...
  for(const key of keys){
    ...
    if(playMode){
      // ✅ PLAY MODE (for Full-view AutoScroll highlight):
      // render bars as beat boxes so we can flash/highlight like card pages.
      const barsWrap = document.createElement("div");
      barsWrap.className = "fullPlayBars";
      const bars = Array.isArray(sec.bars) ? sec.bars : [{text:""}];

      bars.forEach((bar, idx)=>{
        const wrap = document.createElement("div");
        wrap.className = "bar fullPlayBar";
...
if(fullMount){
  ...
  renderFullSongFlow(p, fullMount, { playMode: wantFullPlayMode });
