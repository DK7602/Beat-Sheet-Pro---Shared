...
function getNotesTextAndCaret(editor){
  try{
    const sel = window.getSelection?.();
    if(!editor || !sel || !sel.rangeCount) return { text:"", caret:0 };

    const focusNode = sel.focusNode;
    const focusOffset = sel.focusOffset || 0;
    if(focusNode && !editor.contains(focusNode)){
      return { text:(editor.innerText || ""), caret:0 };
    }
    ...
    const walk = (node)=>{
      ...
      if(node.nodeType === Node.TEXT_NODE){
        ...
        if(node === focusNode){
          caret = text.length + Math.max(0, Math.min(focusOffset, value.length));
        }
        text += value;
        return;
      }
      ...
      if(tag === "BR"){
        if(el === focusNode) caret = text.length;
        text += "\n";
        return;
      }
      ...
      if(el === focusNode){
        const upto = Math.max(0, Math.min(focusOffset, childNodes.length));
        for(let i = 0; i < upto; i++) walk(childNodes[i]);
        caret = text.length;
        for(let i = upto; i < childNodes.length; i++) walk(childNodes[i]);
      }else{
...
function wireNotesEditor(){
  ...
  ed.addEventListener("focus", ()=>{ saveNotesSelection(); scheduleNotesRhymeRefresh(); });
...
document.addEventListener("selectionchange", ()=>{
  if(notesOpen && els.notesEditor && document.activeElement === els.notesEditor){
    saveNotesSelection();
    scheduleNotesRhymeRefresh();
  }
}, true);
