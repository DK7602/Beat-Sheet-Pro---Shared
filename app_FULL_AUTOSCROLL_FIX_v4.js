...
function pickPerfLyricLine(p, barText){
  ...
  // Otherwise, prefer the last eligible lyric line (closest to where the user is writing).
  let lastEligible = "";
  let lastEligibleWithSlash = "";
  ...
  for(let i=0; i<lines.length; i++){
    ...
    lastEligible = t;
    if(t.includes("/")) lastEligibleWithSlash = t;
  }

  if(lastEligibleWithSlash) return lastEligibleWithSlash;
