...
function setAutoScroll(v){
  ...
  if(turningOn){
    ...
    if(p){
      ...
      if(visibleKey === "full"){
        const t = getFirstVisibleFullPreviewTarget();
        if(t && t.secKey){
...
function getFullPreviewBarEl(secKey, barIdx){
  try{
    const fullPage = getActiveRealPageEl("full");
    if(!fullPage) return null;
    const esc = (window.CSS && CSS.escape) ? CSS.escape : (s)=>String(s).replace(/[^a-zA-Z0-9_\-]/g,'\\$&');
    const s = esc(secKey);
    return fullPage.querySelector(`.fullSection[data-sec-key="${s}"] .fullLineBar[data-bar-idx="${barIdx}"]`);
  }catch(_e){
    return null;
  }
}

function getFirstVisibleFullPreviewTarget(){
  try{
    const fullPage = getActiveRealPageEl("full");
    if(!fullPage) return null;
    ...
    const bars = Array.from(fullPage.querySelectorAll(".fullLineBar[data-bar-idx]"));
...
function flashBeatOnAllFullPreviewBars(beatInBar){
  try{
    const fullPage = getActiveRealPageEl("full");
    if(!fullPage) return;
    const bars = fullPage.querySelectorAll(".fullLineBar");
    bars.forEach(bar=>{
    ...
    setTimeout(()=>{
      ...
      if(!full2) return;
      full2.querySelectorAll(".beat.flash").forEach(b=>b.classList.remove("flash"));
    }, 90);
  }catch(_e){}
}
...
function clearOldActiveBar(){
  ...
  if(oldPage){
    oldPage.querySelectorAll(".bar.barActive, .fullLineBar.barActive").forEach(el=>el.classList.remove("barActive"));
...
function setActiveBarDOM(pageKey, barIdx){
  ...
  const bar = page.querySelector(`.bar[data-bar-idx="${barIdx}"], .fullLineBar[data-bar-idx="${barIdx}"]`);
  if(!bar) return null;
...
function flashBeatOnAllBars(pageKey, beatInBar){
  ...
  const bars = page.querySelectorAll(".bar, .fullLineBar");
  bars.forEach(bar=>{
    const beats = bar.querySelectorAll(".beat");
    if(beats && beats.length >= 4){
      beats.forEach(b=>b.classList.remove("flash"));
...
function syncHighlightAndScroll(pageKey, globalBarIdx, beatInBar, seqOffset=0){
  ...
  if(pageKey === "full"){
    if(!autoScrollOn){
      flashBeatOnAllFullPreviewBars(safeBeat);
      return;
    }
    ...
    const barEl = getFullPreviewBarEl(target.secKey, target.barIdx);
    if(!barEl) return;
...
function renderFullAutoPreviewForSection(p, key, previewEl){
  if(!previewEl) return;
  const sec = p.sections[key];
  const bars = (sec?.bars || []);
  previewEl.innerHTML = "";
  ...
  for(const key of keys){
    ...
    const commit = ()=>{
      ...
      tmr = setTimeout(()=>{
        ...
        // keep FULL preview in sync
        try{ renderFullAutoPreviewForSection(p, key, preview); }catch(_e){}
    ...
    preview.className = "fullSectionAutoScrollPreview";
    preview.dataset.secPreview = key;
    renderFullAutoPreviewForSection(p, key, preview);
...
const mountEl = pageEl ? pageEl.querySelector(".sectionMount") : null;
    ...
    try{
      ...
      const prev = fullPage?.querySelector(`.fullSection[data-sec-key="${key}"] .fullSectionAutoScrollPreview`);
      if(prev) renderFullAutoPreviewForSection(p, key, prev);
    }catch(_e){}
